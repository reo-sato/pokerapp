...
- **カードの物理的な載せ替え（ミスディール）は「消えたこと」だけで判断しない**（ADR-0054 / ADR-0058）。
  ...
  前の札が `redeal_confirm_sec` 見えないときだけ。flop は全部消えたとき、6 枚目は消えた札がちょうど 1 枚のとき）。明示コマンド（`cb` / `cs`）も使える。そのハンドで席に記録した札はボードの札にしない
