...
| **R0–R5**（R1 実装済 / 他は提案） | rules-aware hand reconstruction（**hand core 改善トラック**, S 系列と直交） | pokerkit を live ルール権威に / actor 推定（手番 prior × sensor + silent-fold 合成）/ `apply_corrections`（合法手制約）/ 決定的 record/replay + golden fixtures。ADR-0009（提案）/ ADR-0010（R1 実装済）。**R1 record-only ✅** → R2 pokerkit engine → R3 推定/訂正/融合 → R4 contracts → R5 freeze + session 統合 |
