...
│   ├── events.py                  ← AudioEvent, CameraEvent, RFIDEvent, ManualActionEvent 他
│   ├── game_state.py              ← GameStateManager (スタック/ポット/ターン管理)
│   ├── hand_log.py                ← ActionRecord, HandSummary, PotSettlement, RevealedHand
│   ├── settlement.py              ← pot/side-pot 分配 + hand rank 評価 (pokerkit)
│   ├── hand_finalizer.py          ← HandFinalizer (showdown → pot settlement 確定)
│   ├── showdown_tracker.py        ← ShowdownTracker (公開ハンド追跡)
│   ├── hand_boundary.py           ← HandBoundaryDetector (audio/RFID から hand window 検出)
...
│
├── integration/
│   ├── engine.py                  ← IntegrationThread, calc_confidence(), manual_queue
│   ├── action_inference.py        ← BettingState, infer_action(), infer_action_distribution()
│   ├── action_order.py            ← ボタン回転 / SB-BB / actor 順 helpers
│   ├── observation_model.py       ← ベイズ尤度 (EvidenceInterval, ActionHypothesis, 観測モデル)
│   └── beam_search.py             ← Beam Engine (K=8, WINNER 後方修正)
│
├── output/
│   ├── json_writer.py             ← セッション JSON ログ + logs/index.json (viewer 用)
│   ├── phh_exporter.py            ← PHHExporter (PHH 形式エクスポート)
│   ├── evidence_log.py            ← EvidenceLogWriter (raw 観測の append-only ログ)
│   ├── replay_hand.py             ← hand window 再生ユーティリティ
│   ├── reconstruct_session.py     ← CLI: session を再構成し online↔offline diff
...
| contract test | jsonschema ≥ 4.0 | `tests/test_contracts.py`（schema↔fixture 検証） |
| テスト | pytest ≥ 7.0 | |
...
### Hand logger エンジン（実装済 — 詳細は worklog / ADR 参照）

ブランチ統合により、hand logger コアは以下を実装済みとして取り込んだ
（経緯: `docs/worklog/2026-05-22-branch-consolidation.md`、各 Phase worklog）。

- **ASR パイプライン**: Whisper / Vosk の 2 バックエンド（`audio.engine` で選択）。
  `SpeechNormalizer` が action / seat / 数値（漢数字・ひらがな含む）を正規化する。
- **ボタン回転 / ブラインド自動 post**: `integration/action_order.py` がディーラーボタンを
  毎ハンド左隣の active seat へ自動回転し、SB/BB を自動 post、preflop/postflop の first actor を算出。
  heads-up 特例も対応。
- **BettingState + actor 推定**: `integration/action_inference.py` がストリート単位の
  current_bet / contrib / actor を一元管理。`infer_action()` が amount_only 発話から
  BET/CALL/RAISE を補完し、`is_round_closed()` で betting round 終了時に 1 街自動 advance（Phase 5-K）。
- **手動アクション入力（エンジン経路, ADR-0001/0002）**: GUI の手動入力は `ManualActionEvent` を
  `IntegrationThread.manual_queue` に push し、音声経路と同じ品質で処理する。GUI スレッドから
  `GameStateManager.apply_action` を直叩きしない。actor mismatch は **strict reject**（live state を
  汚さず `on_manual_rejected` を発火）。
- **ベイズ推論（v6.0+ M1–M3）**: `output/evidence_log.py` が raw 観測を append-only 記録、
  `integration/observation_model.py` が観測尤度（EvidenceInterval / ActionHypothesis）、
  `integration/beam_search.py` が K=8 の beam で時刻整合な最尤アクション列を推定し WINNER で後方修正。
- **pot settlement**: `core/settlement.py` が pokerkit で hand rank 評価し side-pot / split を分配。
  `core/hand_finalizer.py` が showdown → settlement を確定（**ハンド内のポット分配**であり、
  Future Scope の `session_settlement`＝店への精算とは別概念）。
- **hand reconstruction（advisory）**: `core/hand_reconstructor.py` が evidence log から hand を
  再構成し online ログと diff、`patch_proposal.py` が修正提案を出す（apply は手動・永続化なし）。
  CLI: `output/reconstruct_session.py` / `inspect_reconstruction.py`。

> これらは hand logger 内部の **実装済み** 機能。`docs/contracts/` の凍結対象（player/session/
> ledger/settlement schema）とは別レイヤであり、Future Scope の registry/ledger 計画が設計の核。
...
- 大文字小文字・全半角の厳密同一視は **今回 scope 外**（将来検討、`docs/issues/0004` 参照）。
...
| GUI ダッシュボード | 🔨 部分実装 | `gui/dashboard.py`（手動アクションパッド含む） |
...
| ベッティングステート / actor 推定 | ✅ 実装済 | `integration/action_inference.py`（amount_only 補完, round-close 自動 advance） |
| Vosk 代替バックエンド | ✅ 実装済 | `audio/vosk_recorder.py`（`audio.engine="vosk"`） |
| 音声正規化 / 数値正規化 | ✅ 実装済 | `audio/speech_normalizer.py` |
| ディーラーボタン自動回転 / SB/BB 自動 post | ✅ 実装済 | `integration/action_order.py` |
...
- **Done criteria**: 共有 ID 契約が ADR 化され（ADR-0005）、schema/fixtures の置き場所と更新手順が
  ...
  - **残（Phase 0b〜）**: `session_id` / `hand_id` の cross-app 形確定（ISSUE-0006）、
...
- **Prerequisites**: session 契約（S2）+ ledger/point schema 凍結 + ISSUE-0003（point 残高の
  source of truth）の決着。
- **Parallel tasks**:
...
- **Blockers**: **ISSUE-0003**（残高 source of truth）。これが決まらないと残高計算の API 契約が
