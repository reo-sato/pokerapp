...
| **ハンドリプレイ UI (ADR-0044 → ADR-0051)** | ✅ 実装済 | **スマホ縦 1 画面・スクロールなし**の GGPoker 風ハンドログ（ADR-0051）: 上半分 = テーブル図（席を外周にリング配置[先頭が手前・時計回り]・記録がある席は全員分ホールカード・勝者は緑枠 + 🏆・中央にボード/ポット/サイドポット/ブラインド。**最終状態で固定**）、下半分 = プリフロップ/フロップ/ターン/リバーの **4 列**（列見出しは新規ボード + 街終了ポット、行は 2 段 = `席番号 + プレーヤー名` / **種別ごとに色を付けた**原語表記[Fold/Check/Call/Bet/Raise/All-in、アグレッション順に 灰→鋼→青→緑→橙→赤] + 短縮金額[1.5k/12.2k]、要確認/訂正済は左ストライプ + `!`/`✎`）。溢れた列だけ列内スクロール（画面は動かない）。**プリフロップ fold の席は減光 / 本人の席とアクション行は琥珀で強調（mobile のみ）/ プリフロップ列は fold 行を出さない**（ADR-0051 追記 D7/D8。名前・色・減光・省略はいずれも描画専用の規則でポット計算・テキスト書き出しは不変）。BTN/ポジションは非表示（データに無く推定は scope 外 = ADR-0044 D3 維持）。正本 `shared/hand_replay/`（`handReplayModel.ts` 純関数 = ストリート分割 + `seatRingLayout` + 短縮表記 + 種別色 + TS tests 17 / `HandReplay.tsx`）を `scripts/sync_shared_ui.py` で mobile/staff 両アプリへコピー配布、drift は `tests/test_shared_ui_sync.py`（CI）。mobile HandDetail（ScrollView なし）+ staff ハンドタブ drill-in に搭載。staff hands read = `GET /api/staff/sessions/{sid}/hands`（訂正適用済, `list_session_hands` + `ViewerApiClient.list_session_hands`）。ステップ再生・ストリート連動は scope 外（ADR-0051 D2） |
