...
### `core/patch_apply.py` (Phase 5-G / 5-H 実装済)
...
#### Phase 5-H: ``HandPatchProposal`` の GUI 表示用 view helper

apply とは独立した read-only な detail view 用のエントリも同じモジュールに同居:

- ``FieldDiffView(field, online_repr, offline_repr, is_applicable, note)``:
  1 field 分の patch proposal を GUI 表示用に整形した dataclass。
  ``is_applicable`` は ``field in PATCH_APPLY_FIELDS`` で決まる
- ``_format_value_for_view(value) → str``: 値を deterministic に文字列化
  (``None`` → ``"—"``、int キー dict は昇順 sort、str キー dict は挿入順、
  list は ``"[v1, v2]"``、不明型は ``repr``)
- ``summarize_patch_proposal_for_view(summary, proposal) → list[FieldDiffView]``:
  proposal を view list に投影。proposal が None / fields 空 / 全 entry が
  malformed なら ``[]``。field 順は ``proposal.fields`` の順を維持
  (= deterministic 出力)。dataclass / dict 両形の proposal に対応

**Phase 5-H detail view 出力例** (GUI の Show details ボタン押下時の Toplevel
テキスト)::

    Hand #5 patch proposal — 3 field(s)  (patch applied — applied_fields=resolution_type,seat_payouts)
    ...
    [applies] seat_payouts
      online:  {1: 300}
      offline: {2: 600}

``summary`` 引数は現状未使用 (= 将来 "current value" カラム追加用の前方互換)。
...
→ 641 passed  (test_vision.py は cv2 未インストールのため収集エラー、既知問題)
```
...
```bash
pytest tests/ -v --ignore=tests/test_vision.py                                   # 全 suite: 641 件 pass (280 baseline + 66 M1–M3 + 6 Phase 1 + 20 Phase 2-A + 11 Phase 2-B + 19 Phase 2-C + 14 Phase 3 + 10 Phase 4-A + 11 Phase 4-B + 14 Phase 4-C1 + 33 Phase 4-C2 + 27 Phase 5-A + 13 Phase 5-B + 9 Phase 5-B+ + 10 Phase 5-C + 10 Phase 5-D + 10 Phase 5-E + 13 Phase 5-F + 42 Phase 5-G + 23 Phase 5-H)
...
**Phase 5-H 完了済み (GUI で patch proposal の field 単位 detail view)**:
- ✅ ``core/patch_apply.py`` に Phase 5-H 用エントリを追加:
    - ``FieldDiffView(field, online_repr, offline_repr, is_applicable, note)``
      dataclass
    - ``_format_value_for_view(value)`` で値を deterministic に文字列化
      (None → ``"—"``、int キー dict は昇順 sort、str キー dict は挿入順、
      list は ``[v1, v2]`` 形)
    - ``summarize_patch_proposal_for_view(summary, proposal) → list[FieldDiffView]``
      で proposal を view list に投影 (dataclass / dict 両形対応)
- ✅ GUI (``gui/dashboard.py``): history frame row=2 column=1 に "Show details"
  ボタン (Apply patch は column=2 に右シフト) を配置。``_cmd_show_patch_details``
  ハンドラが ``summarize_patch_proposal_for_view`` を呼び、
  ``_format_patch_detail_lines`` でテキスト行リストに整形 →
  ``_create_patch_detail_window(title, lines)`` で Toplevel を生成
- ✅ ``_create_patch_detail_window`` は最小限の Tk Toplevel + CTkTextbox + insert
  実装で、テスト時は ``dash._create_patch_detail_window = MagicMock()`` で
  差し替え可能 (= ユニットテストで Toplevel 実体を作らずに引数検証できる)
- ✅ whitelist field は ``[applies]``、非 whitelist field は ``[skip]   ``
  マーカーで一目で区別。apply 済 hand では header に
  ``(patch applied — applied_fields=...)`` を付与
- ✅ Show details は Apply と **完全独立**: read-only で
  ``apply_patch_proposal`` を呼ばず、``_last_summary_by_hand_id`` も触らない。
  apply 前後どちらでも開ける

**Phase 5-H スコープ外 / 既知の注意事項**:
- **apply 後の detail view では online_repr が "pre-apply の online" を示す**:
  ``proposal.online`` は proposal 生成時に固定されているので、apply 後でも
  online_repr の値は変わらない。一方 ``offline_repr`` は提案された
  (= 既に summary に反映済みの) 値。これにより apply 後の detail view で
  「online と offline が両方とも `showdown`」のように見える混乱は起き **ない**
  (= online は依然として元の online 値を表示する)。
  正確な "現状値 (= apply 後の summary の値)" カラムを追加するのは将来課題
  (``proposal.original_online`` の保持や ``summary`` 経由の dual-column 表示)
- **detail view は read-only**: clipboard コピーや個別 field の apply は無し。
