...
- **カードの物理的な載せ替え（ミスディール）は「消えたこと」だけで判断しない**（ADR-0054 / ADR-0058）。
  ...
  差し直しは、前の札が消えた直後に**前の札を読んでいたリーダーの上**へ置かれたときだけ。flop は全部消えたとき、
  6 枚目は消えた札がちょうど 1 枚のとき）。明示コマンド（`cb` / `cs`）も使える。そのハンドで席に記録した札はボードの札にしない
