...
- **カードの物理的な載せ替え（ミスディール）は「消えたこと」だけで判断しない**（ADR-0054 / ADR-0058）。
  ...
  「前の札が `release_sec` 以上見えず、**新しい札が載り続けた**」ときだけ自動で反映する（ボードはストリートの札が
  全部消えたときだけ）。明示コマンド（`cb` / `cs`）も使える。そのハンドで席に記録した札はボードの札にしない
