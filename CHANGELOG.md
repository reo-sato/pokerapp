...
- **ボードは 1 枚だけの差し直しに対応**（ADR-0058 追記 / 追記 2）。前の札を取り、**同じ場所**に新しい札を置けば、
  ...
  扱い、新しい札は次の位置。別の場所の札が読めない間に次の札が来ても次の位置のまま。一度読めなくなって戻った札は
  同じリーダー・6 秒で判断する。flop 全体を外したときは時間によらず差し替え、5 枚埋まったあとの 6 枚目は消えた
  1 枚を抜いて詰める。**前の札を先に取ってから新しい札を置く**（先に置くなら約 2 秒以内に前の札を取る）。
...
- 記録: `RFIDEvent.replaces`（差し替えた前の札）を additive 追加し、`reconstruction_event` schema を `0.4` に。
  契約 `rfid-usb-ccid.md` v1.5〜v1.7 §4（firmware の要求は不変）。
