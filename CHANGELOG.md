...
### Added (店舗の進行に合わせる: 役名でハンド終了・席の参加と休み・ブラインドの変更 — オーナーの回答, 2026-09-26)

- **ショーダウンの役名**: ディーラーが「ツーペア」「フラッシュ」などの勝った役名を言うと（「ウィナー」は言わない
  運用）、「ハンド終了」と同じくハンドを終え、RFID の手札とボードの判定と突き合わせる。一致すれば確定、判定と
  違えば要確認（役名に合う席が 1 つだけならその席の勝ち = `winner_source: announced`）。手札が読めていない席が
  あっても、役名から勝者を決める（要確認）。役名の前に閉じていないベッティングは聞き取れなかったとみて閉じる。
  記録に `announced_hand`（hand schema `1.5`）、events.jsonl に `hand_name`（reconstruction_event `0.6`）。
- **席の参加・休み**: `name <席> -` で空席にした席は次のハンドから配られない。`name <席> <名前>` で戻る。
  **スタック 0 の席（バースト）は `r <席> <金額>` で買い足すまで配られない**（pokerkit はスタック 0 を受け付けず、
  そのまま配ると新しいハンドが始まらなかった）。ボタンは参加している席の中で回る。配られる席が 2 つ未満なら
  ハンドを始めずに知らせ、買い足し・参加で始まる。ハンドの記録の `players` は配られた席だけ。
- **ブラインドの変更**: `blinds <SB> <BB>` で次のハンドから（トーナメントのレベル上昇。店舗 iPad からは control
  `set_blinds`、席の参加・休みは `sit_in` / `sit_out`）。ハンドの記録の `blinds` はそのハンドのもの。
- **レイズの額は常にトータル**（上乗せ分は言わない）: 最小レイズ未満の額を「追加額」と読み替えず、聞き違いとして
  丸めて要確認にする。
- **確認型の発話はアクションにしない**（仕様 FR-16/17, §7）: 「コールですか？」「レイズ 2400 でよろしいですか？」の
  ような疑問形は、ディーラーがプレイヤーに確かめている言葉なので読まない（CLI には「確認の発話」と出る、
  transcripts.jsonl に `question: true`）。確認のあとの確定の発話（「コール」）だけを読む。
- **ディーラーボタンの手動移動**（仕様 FR-05g）: `button <席>` で次のハンドのボタンを直す（control `set_button`）。
  通常の進み方と違う席になったら知らせる（仕様 §9）。休み・スタック 0 の席を飛ばす回転は仕様 FR-05b どおり。
- **無言の連続**（連続する席が同じコール / チェック / フォールドをしたら 2 回目以降は言わない = 店舗のディーラー）:
  言われなかったチェック / コールは、直前に言われたアクションと同じなら要確認にしない（`silent_repeat`）。
  「チェック」「コール」のあとのベット / レイズは、いちばん近い手番の人に仮に置き、RFID でフォールドした席を候補
  から外す。仮に置いた人の札がラウンドの途中で離れたら（ベットした人に手番は無い）無言のチェック / コールだった
  とみて記録を組み直す。全員がコールしてラウンドが終わったときは、誰がベットしたかを決められないので、その
  ベットを要確認にする（`silent_run_ambiguous(席…)`）。席・ポジションを言ったベット / レイズは、間の人を無言の
  連続（札が離れていればフォールド）とみる（合成フォールドで埋めない）。
- **ベットに全員が降りたあと勝った人が素早く札を投げても大丈夫**（オーナー）: 仮のベットの席の札が離れても、
  ほかの候補の席が先に降りていれば解釈し直さない（候補の中で最後に札が離れた人がベットした人）。勝った人の
  離脱を「フォールドかショーダウンか確認」の要確認にしない。リバーでは「コール」の聞き落としの可能性があるので
  役名・「ショーダウン」を待ち、無ければフォールドで確定する。「フォールド」と聞こえたのに札が席に残っている
  ときは、札が離れたらその発話の時刻のフォールドにする（勝った人が先に投げても順序が保てる = 発声を省略しない
  運用が効く）。

### Fixed (配る前に `n` で始めたハンドを読まなかった・雑音の長い書き起こし — 店舗の 5 回目の通しテスト, 2026-09-25)

- **配る前に `n` / 「ハンド開始」で始めたハンドが、配るまでの間（空の卓）に「片付け = プレーの終わり」と
  みなされていた**。そのまま配っても戻らず、ボードの札と声を読まずに、ショーダウンで前に出した札を
  プリフロップのフォールドにしていた。まだ何も起きていないハンド（配る前・配り直し）の空の卓はプレーの
  終わりにしない。
- `n` のあと配るまでの間にボードのリーダーの上で札を混ぜても、その札をボードにしない（手札が 1 枚も
  届いていないハンドのボードは読まない。最初の手札が届いたら、配る前に読んだ札の位置を捨てて本物の flop を
  1 枚目から数える）。
- **雑音への幻聴**: Whisper がプロンプトの語を繰り返す長い文（「シート4 レイズ 2千、コール、チェック、…」）を
  1 回 15〜18 秒かけて書き、あとの発話の反映が 50 秒遅れていた。
  - 声が無い音（札を混ぜる音など）は Whisper にかけない（Silero VAD, `audio.vad_threshold` 既定 0.5、0 で
    無効）。記録（transcripts.jsonl, `no_speech: true`）にだけ残し、CLI には出さない。
  - 書き起こしの長さに発話の長さに応じた上限を付け、長すぎる書き起こしは雑音として捨てる。
  - CLI では雑音の文の頭 20 文字だけを出す（全文は transcripts.jsonl）。
  - `tools/audio_check.py listen` に「声ではない音」を表示（`--vad-threshold` で試せる）。

### Added (マイクに声が入らないときの知らせ, 2026-09-25)

- プレー中に 60 秒以上マイクに声が入らなければ、CLI に `●` で「マイクの電池・受信機・音量を確かめて」と
  1 ハンドに 1 回知らせる（ワイヤレスマイクの電池切れで、ハンドが丸ごと音声なしで記録された）。

### Fixed (ショーダウンで前に出した札をフォールドにしていた — 店舗の 4 回目の通しテスト, 2026-09-25)

音声のアクションが 1 件も記録されなかったハンドで、記録がプリフロップのまま止まり、リバーのあとにショーダウンで
前に出した札を「手番の人のフォールド」にしていた（最後まで残った席が負けになった）。
- **ボードの札でストリートを進める**: 札が置かれた時点で前のラウンドが閉じていなければ、札が離れていた人は
  フォールド、残っている人はチェック / コール（聞き取れなかったとみて補う, 要確認）で閉じる。札が置かれた
  時刻より前に話し始めた発話は先に反映する。記録（`events.jsonl`, `kind: street`）で replay も同じになる。
- **リバーでベットが無いときの札の離脱はチェック**（ショーダウンに向けて前に出した, CLI は `← 札を前に出した
  （ショーダウン）`）。フロップ・ターンでベットが無いときの離脱はフォールド（降りた）。
- リバーで最後のフォールドとみた直後に残った人の札も離れたら、ショーダウン（最後のコールの聞き落とし）とみる。
- 次の配布の時点でボードが 5 枚なら、閉じていないリバーを閉じて手札で勝者を決める。
- 次のハンドの札が同じ席に置かれたのを「札が戻った」と取り違えない（そのハンドの手札が戻ったときだけ）。

### Changed (フォールドは席の札の離脱で決める — オーナー決定, 2026-09-25)

一般的なディーラーの発声のままで記録を成立させるための設計監査（`docs/worklog/2026-09-25-voice-design-review.md`）
を受けた変更。
- **フォールドは RFID で決める**: 席の札が 3 秒離れたまま戻らない（`engine.fold_absent_sec`）/ 卓の中央
  （ボードのリーダー）を通過 → その席のフォールド。CLI には `← 札が離れた` / `← 札が中央を通過` と出る。
- **音声の「フォールド」は次の手番の人に付けない**。札が離れかけている席があればすぐフォールドにし、札が
  残っていれば記録しない（別の言葉の聞き違い・別の席のこと）。
- **手番でない席の札が離れた = その前の人のアクションが聞き取れなかった**とみて、間の人をチェック / コール
  で補う（要確認、`← 聞き取れなかったとみて補完`）。2 ハンド目は「600」が読めなくても、席4 のフォールド・
  席5 のレイズ 2500・席6 のコールが正しい席に付き、ポットも正しくなる。
- **札が戻ったら、少なくともその時刻まではフォールドではない**: 入れたフォールドを取り消し、そのあとの
  アクションを流し直して記録を組み直す（`●` で知らせる）。
- 札の離脱は、それより前に話し始めた発話を反映してから入れる（認識は数秒遅れるため）。
- ショーダウン（ベッティングが終わったあと）は札を前に出すので、離脱はマックにしない。見せずにマック
  したときはディーラーの「フォールド」で決める（従来どおり）。
- 最後の 1 人を残すフォールドは 10 秒待って確定（音声の「フォールド」・中央の通過・次の配布でも確定）。
  その間に札が戻る・「ショーダウン」と聞こえたら取り消す（ショーダウンなら最後のコールを補う）。
  残った人の札も離れたら要確認。
- 札の離脱・戻り・確定は `events.jsonl` に記録し（`reconstruction_event` 0.5: rfid の `kind` / `observed_at`）、
  replay で同じ記録になる。無効にするなら `engine.rfid_folds=false`（従来どおり音声のフォールド）。

### Added (記録の完全化, 2026-09-25)

- 聞き取った発話を**すべて** `logs/<セッション>.transcripts.jsonl` に残す（アクションとして読めなかったもの・
  雑音として無視したものも。話し始め・認識の時刻・信頼度・読めたアクション）。
- `audio.save_audio=true` で、認識に回した発話の音声を `logs/audio/<セッション>/<話し始めのミリ秒>.wav` に保存
  （聞き違いの原因を確かめるため。卓の会話も入るのでテスト中だけ）。
- 発話の前 0.3 秒の音も認識に回す（以前は 0.064 秒。語頭の小さい子音が切れて短い語が崩れるのを防ぐ）。

### Fixed (店舗の 3 回目の通しテスト 2 ハンド目, 2026-09-25)

- 聞き取った額を **BB の倍数に丸めていた**（100/200 の卓で「2500」が 2400 になった）。丸めの単位をチップの
  最小単位（SB と BB の最大公約数）にした = 言った数字どおり。
- 「チェック、アランド」のように区切って書き起こされたチェックアラウンドも全員のチェックにする。
- 数字だけの発話を仮名でも読む（「ロッピャク」= 600、「センゴヒャクテン」= 1500）。発話が額だけのときに限る。
- 「次回もお楽しみに」を雑音への幻聴として無視する。

### Added (店舗の 3 回目の通しテスト: 読んだ札を CLI に出す・数字だけのベット・チェックアラウンド, 2026-09-25)

- **CLI に RFID で読んだ札を出す**（`[カード]` 行）: ハンド開始時の手札（あとから 2 枚そろった席も）、
  フロップ / ターン / リバー、配り直しの差し替え、ハンド確定時のまとめ（ボードと全員の手札、手札で判定した
  ときは役）。例 `[カード] ターン 5♥（ボード 3♦ J♠ 7♥ 5♥）`。
- **数字だけの発話はベットかレイズ**（「600点」「2千点です」）。どちらかは状態から決める。いまのベット以下
  （コールの額の言い直し）・最小ベット未満（ショーダウンの「7」など）・ベッティングの外の数字は記録せず、
  理由を `●` で知らせる。「ポット 2千点」「残り 1500」「5 6 7」のような発話はアクションにしない。
  コール・チェック・フォールドの前後に区切って言った額は別の人のベット（「2千点、コール」= ベット 2000 → コール）。
  キーボードからも `600` と打てる。
- **「チェックアラウンド」= まだ動いていない全員がチェック**（オーナーの説明）。前回は 1 人分のチェック
  にしかならず、以降の手番がずれていた。
- 前のストリートで言われた「コール」の言い直し（「コール」のあとの「600点コールです」がターンの札より前）は、
  次のストリートのチェックにしない。ボードを RFID で読んでいるハンドで、いまのストリートの札より前に話し始めた
  ものが対象。
- 雑音への幻聴を増やして無視: プロンプトの語を順に並べたもの（「コール、チェック、フォールド、オールイン、…」
  = 前回「シート4 レイズ 2千」として記録された）と「ご視聴ありがとうございました」等の字幕の定型句。
