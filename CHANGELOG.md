...
- **ボードは 1 枚だけの差し直しに対応**（ADR-0058 追記）。前の札を取り、**同じ場所**（前の札を読んでいた
  リーダーの上）に新しい札を置けば、その位置の札が替わる（flop の 1 枚を差し直しても turn にならない）。前の札が
  消えてから 30 秒以内（`rfid.redeal_window_sec`）に置いた札が対象で、反映は前の札が消えて約 6 秒後。前の札が
  戻れば読み落ちとして扱い、新しい札は次の位置。別の場所の札が読めない間に次の札が来ても次の位置のまま。
  flop 全体を外したときは時間によらず差し替え、早すぎた turn を外して間を空けて配り直した場合は river が来た
  時点で並びを直す。**前の札を先に取ってから新しい札を置く**（先に置くなら約 2 秒以内に前の札を取る）。
...
- 記録: `RFIDEvent.replaces`（差し替えた前の札）を additive 追加し、`reconstruction_event` schema を `0.4` に。
  契約 `rfid-usb-ccid.md` v1.5 / v1.6 §4（firmware の要求は不変）。
