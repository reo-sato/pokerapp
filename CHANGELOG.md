...
### Changed (リプレイ 4 列にプレーヤー名 + アクション種別色 = ADR-0051 追記 D8)

- **アクション行にプレーヤー名を表示**。列幅（約 88px）に名前・アクション・金額は 1 行で収まらないため
  行を **2 段**にした（1 段目 = `席番号 + 名前`、2 段目 = アクション名 + 金額 + `!`/`✎`）。名前は
  `action.player_name` → 席→参加者名 → `席N` の順に解決するので、古い記録でも空欄にならない。
- **アクション種別ごとに色**を付けた（`ACTION_COLORS` / `actionColor`）。色相は**アグレッション順**で
  fold 灰 → check 鋼 → call 青 → bet 緑 → raise 橙 → all-in 赤。色は補助チャンネルで、テキストラベル
  （Fold/Check/Call/Bet/Raise/All-in）は常に併記する。金額は種別色と競合しないよう青から通常テキスト色へ。
- 本人のアクション行は地の琥珀（D7）に加えて 1 段目の名前も金色。
- **テキスト書き出し（📤 共有 / コピー）は不変**（日本語ラベルのまま、色も名前も描画のみ）。
- 検証: mobile 39 / staff 47 tests + typecheck green、E2E 8 passed、pytest 836、ruff clean。
  400×860 実測で 1 画面維持（3 アーキタイプとも `scrollHeight == clientHeight`）。

### Changed (リプレイの強調・省略規則 = ADR-0051 追記 D7)

1 画面リプレイの俯瞰性を上げる 3 点。いずれも**描画のみ**の規則で、ポット計算・テキスト書き出しは不変。

- **プリフロップで fold した席をグレイアウト**（ハンドに関与していない席を沈める）。
  ポストフロップの fold は減光しない（フロップまで参加した席はハンドの一部）。
- **本人の席を琥珀の地 + 金の縁で強調**（mobile のみ。staff は「本人」が存在しないため強調なし）。
  彩度の高い黄色はホールカード・文字の可読性を落とすためダーク基調になじむ暗い琥珀を使う。
  勝者かつ本人のときは地は琥珀のまま縁だけ勝者の緑を優先。**本人のアクション行**も 4 列側で薄い琥珀に。
- **プリフロップ列では fold 行を出さない**。大半が降りるプリフロップで実質的な攻防が埋もれるため
  （降りた席はテーブル図の減光で示すので情報は失われない）。フィルタは描画専用で、
  `buildReplayModel` のポット境界も `handReplayText.ts` の書き出しも全アクションを見たまま。
- 判定は純関数（`preflopFoldedSeats` / `visibleColumnActions`）に切り出し +3 テストで回帰ロック。
- 検証: mobile 38 / staff 46 tests + typecheck green、E2E 8 passed（無改修）、pytest 836、ruff clean。
  400×860 実測で 1 画面維持。
