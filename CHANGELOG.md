...
### Fixed (店舗の 2 回目の通しテスト: シャッフルでハンドが始まる・聞き取りが数分遅れる, 2026-09-25)

- **ハンドは、2 席以上に手札 2 枚ずつが約 1.5 秒載り続け、ボードのリーダーが空のときだけ始まる**
  （RFID のいま載っている札で判断）。シャッフル・ウォッシュでリーダーを一瞬通った札や、ショーダウンのあと
  席に残った前のハンドの札では始まらない。
- **ボードは手札が配られてから読む**。配る前（シャッフル中）と、プレーが終わったあとのボードの札は無視する。
  ショーダウン待ちのボードは、次のハンドのウォッシュで差し替わらない（前回はボード 5 枚が差し替わっていた）。
- **音声はプレー中（配布〜ハンドの確定）だけ認識に回す**。ハンドの間の会話で認識待ちがたまらない。ショーダウン
  のあと誰も何も言わなくても、卓に札が無い状態が 15 秒続いたら聞き流す。
- **聞き取りの遅れを減らした**: 自信の低い音声を何度もやり直す設定をやめた（雑音で 1 発話 10〜80 秒かかって
  いた）。雑音に対して Whisper が例文（「…チョップ などの言葉が含まれます」）をそのまま書き起こしたものは
  「雑音として無視」し、アクションにしない（前回はレイズ・コール・ウィナーとして記録されていた）。
  CLI の聞き取り行に、10 秒以上遅れたときは「（N 秒前の発話）」と出す。
- 「なべとなっております」の「ベト」をベットと読まない（短い書き起こしゆれの語は、前後が区切り・数のとき
  だけアクションにする）。
- config（任意）: `audio.speech_rms`（有音とみなすレベル）/ `audio.beam_size` / `audio.temperature_fallback`。

### Added (手札を配ったらハンド開始・勝者の自動判定・仮名の金額 — ADR-0062 / ISSUE-0038, 2026-09-25)

店舗の通しテストで、`n` /「ハンド開始」を言わない運用のためハンドが一度も始まらなかったことへの対応。

- **手札を配ると新しいハンドが始まる**（2 席以上に札が置かれた時点。`n` /「ハンド開始」は不要）。
  - CLI に「● ハンド N 開始（手札が配られました / ボタン 席X）」と出る。配った札はそのハンドの手札になる。
  - 配ったあとに「ハンド開始」と言ったり `n` を打ったりしても、二重には始めない。
  - 前のハンドが終わっていなければ、先に確定してから始める。
  - 配る前に言ったアクション（前のハンドの最後のコールやマック）は、認識が遅れても前のハンドに入る。
- **勝者を自動で決める**。
  - ほかが全員フォールドしたら、残った人の勝ちで即確定。
  - ショーダウン（2 人以上残ってベッティング終了）ではすぐに決めない。「フォールド」と言うと、残っている中で
    **一番アウトオブポジションの人から順に見せずにマックした**とみなし、最後に残った人の勝ち。
    - 見せずにマックした人は、手札が強くても負け。手札で見てマックした側が強かったハンドは「要確認」にする。
    - 席を言えば（「シート5 フォールド」）その席。
  - 誰もマックしなければ、「ハンド終了」と言ったとき（または次の手札を配ったとき）に RFID の手札とボードで判定する。
    - side pot の勝者が別でも、引き分けでも正しく配る。
  - 手札が読めていなければ「w <席> で入力してください」と出る。
  - 設定: `engine.auto_new_hand` / `engine.auto_winner`（既定 on。今の config.json のままで有効）。
- 聞き取り:
  - 「ベトナナ」（ベット 7）のように**仮名で書き起こされた金額**を読む（ナナ = 7、ロッピャク = 600 など。
    アクションの語の直後だけ）。
  - 「ゴール」をコール、「ベト」をベットとして読む。「ハンド終了」を追加。
- 記録: ハンドに `winner_source`（fold / cards / estimated）と `showdown`（見せた手札の役）を追加
  （`hand` schema 1.4、`action` 1.5。追加のみ）。

### Added (好きな番号の席を使う, 2026-09-25)

- ハンドロガーの起動時の「席」に、人数（`6` = 1〜6 番、従来どおり）のほか、**使う席番号**を空白区切りで入れられる
  （`2 4 5 8`。卓の空いている席を飛ばして座るとき）。名前・スタックはその番号の席ごとに訊き、1 ハンド目の
  ボタン席も使う席の中から選ぶ。手番・ポジション（SB/BB/BTN…）・RFID の席のリーダーは席番号どおりに対応する。
  全角の数字や「、」「,」区切りも可。

### Fixed (音声: 続けて言ったアクション・「ベッド」・短い言葉・遅れで捨てる発話 — ADR-0061 / ISSUE-0037, 2026-09-25)

店舗 PC でのマイクの実測と「実運用では席番号を言わない」を受けて。

- **続けて言った複数のアクションを、言った順に分けて記録する**（「フォールド、フォールド、コール」→ 3 つ）。
  以前は先頭の 1 つだけを記録して残りを落としていた。「チェックレイズ 1200」は 1 つのレイズとして読む。
  CLI で 1 行に続けて打った場合も同じ。
- 「ベッド」をベットとして読む（Whisper の書き起こしゆれ。以前は読めず、後ろのレイズが誤った席・金額で記録された）。
  続けて言った 2 つ目の「フォールド」が「ホールド」と書き起こされたので、「ホールド」もフォールドとして読む。
  チェックレイズは「レイズ 2000」とだけ言う（「チェック、レイズ」と区切られると 2 人のアクションと区別できない）。
- 「チェック」のような短い言葉を捨てない（認識に回す有音の下限を 0.3 → 0.15 秒、config `audio.min_speech_sec`）。
  `audio_check listen` は、短すぎて捨てた音を「短い音 N 秒 — 認識に回さず」と表示する。
- **認識が遅れても発話を捨てない**（以前は 8 件たまると古い発話を捨てていた）。プレーの切れ目で追いつく。
- **打った `n` / `w` / 読み上げ文などは、先に言われた発話の反映を待ってから入る**（「聞き取った発話を先に反映して
  います… 残り N 件」）。前のハンドのアクションが次のハンドに入らない。
- docs: 席番号を言わない運用の読み上げ方（全員のアクションをフォールドも含めて順に、勝者は `w <席>`）。

### Added (音声テストの見える化 — ADR-0060, 2026-09-25)

お客さん向けの画面を使って音声入力を含めたテストをするための準備。

- **マイク確認ツール `tools/audio_check.py`**: `list`（録音できるデバイスの番号・方式・16 kHz で開けるか。config の
  番号に印）/ `level`（入力レベルのバー。発話として拾えるか）/ `listen`（本番と同じ経路で聞き取り、発話ごとに
  「何と聞こえた → どのアクションになったか」と認識にかかった秒数）。初回の音声認識モデル（約 1.5 GB）の
  ダウンロードも `listen` で本番前に済ませられる。
- **ハンドロガー（`--cli`）が聞き取った文を表示**: `[聞き取り] 「シート3 レイズ 600」→ raise 600 席3`。アクションとして
  読めなかった発話も「アクションとして読めず」と出す（ログファイルにも INFO で残る）。
- ハンドロガーが起動時に**音声認識モデルの読み込み中・所要秒数**と**マイクの状態**（番号と名前 / 開けなかった理由）を
  表示する。モデルを取得できなくても止まらず、音声なしで続ける。
- **ハンドの記録にそのアクションになった発話（`raw_text`）を残す**（action schema 1.4, additive。合成したフォールドには無い）。
- **スマホ画面のテスト表示**: URL に `?test` を付けて開く（例 `http://192.168.1.136:8788/?test`）と、各アクションの下に
  聞き取った文と、補正したときはその内容と理由（「ベットがあるのにチェック → コールにした」など）が出る。
  `?test` の無い端末（お客さん）の表示は変わらない。
- troubleshooting: マイクの確認手順を `audio_check` に書き直し、RDP の音声設定（「リモート コンピューターで再生する」）と
  Windows のマイクのプライバシー設定を追加。

### Added (お客さんがスマホで自分のハンドを見る — ADR-0059 / ISSUE-0036, 2026-09-25)

お客さん向けのハンド閲覧（`mobile/` の web 版 + viewer API）を、店舗の構成（ハンドロガー `--cli`）で使えるようにした。

- **ハンドロガー（`--cli`）が「誰がどの席か」を記録する**（`config.json` の `session_layer.enabled=true`）。
  起動時に入れた名前がお客さんになり（同じ名前は同じ人。空 Enter の席は結び付けない）、ハンドごとに席を記録する。
  **`name <席> <名前>`** で次のハンドから席替え、`name <席> -` で空席（`seat 3 call` は従来どおり読み上げ文）。`q` で終えるとセッションは「終了」になる。
- **viewer API がお客さん向け画面も出す**。お客さんは店の Wi-Fi で **`http://<PC の IP>:8788/`** を開き、名前を
  選ぶだけ（ログイン不要）。セッション → ハンド一覧 → ハンドの詳細（記録した全員分のホールカード・収支）。
  PIN / LINE・Google のログインやハンド訂正の導線は出ない。
- 画面を先に起動しておけば、あとから始めたセッション・ハンドは「↻ 再読込」で出る（画面の再起動は不要）。
- デスクトップのショートカット **「お客さん用 ハンド履歴 (スマホ)」**（`start_viewer.cmd` =
  `--viewer-api --host 0.0.0.0 --port 8788`）。`--viewer-api` に `--host` / `--port` を追加。
- 画面のビルドを同梱（`api/static/player/`。店舗 PC に Node は不要）。`mobile/` を変えたら
  `python scripts/build_player_web.py` で作り直す（忘れると CI が落ちる）。
- `tools/set_config.py <項目> <値>` で config.json の 1 項目を変える（例 `session_layer.enabled true`）。
  config.json は BOM 付きでも読める。
- Windows で hand logger と画面が同じファイルを同時に使っても書き込みを失わないよう、置き換え・読み込みを短く再試行する。
- 当面は記録したホールカードを全員分見せる（本人確認なし・店の Wi-Fi 限定）。セッションごとの公開範囲は今後。

### Changed (RFID を卓の流れに合わせて解釈 — ADR-0058 / ISSUE-0035, 2026-09-24)

店舗の実卓で「フォールドした手札がボードのリーダーの上を通る」「配り直しにディーラーの入力を求められない」ことが
分かったため、本番の起動経路（`python main.py` / `--cli`）での RFID の解釈を変えた。

- **フォールドした手札はボードの札にならない**。そのハンドで席に記録した札を board reader が読んでも位置を与えず、
  その席の「マック」として記録する。卓モニタに「マック」と出る。
- **ボードの札は約 2 秒載り続けてから数える**（`rfid.commit_sec`）。一瞬通過した札（席で読めていない手札など）は
  数えない。配った時刻は最初に見えた時刻のまま。
- **配り直しは入力なしで反映**。前の札が約 6 秒見えず（`rfid.release_sec`）、新しい札が載り続けたら差し替える。
  全員の札を配り直して前の手札が別の席に来た場合も移す。差し替えたハンドは `needs_review`。
- **ボードは 1 枚だけの差し直しに対応**（ADR-0058 追記 / 追記 2）。前の札を取り、**同じ場所**に新しい札を置けば、
  その位置の札が替わる（flop の 1 枚を差し直しても turn にならない）。flop の札は前の札を読んでいたリーダーの上・
  消えてから 30 秒以内（`rfid.redeal_window_sec`）、**最後に配った turn / river は同じか隣のリーダー・時間は問わない**
  （店舗の実卓で turn の差し直しが river になっていた = turn の位置がリーダーの境目にある）。反映は前の札が消えて
  約 3 秒後（`rfid.redeal_confirm_sec`。以前は 6 秒で「反映が遅い」と報告があった）。前の札が戻れば読み落ちとして
  扱い、新しい札は次の位置。別の場所の札が読めない間に次の札が来ても次の位置のまま。flop 全体を外したときは時間に
  よらず差し替え、5 枚埋まったあとの 6 枚目は消えた 1 枚を抜いて詰める。**前の札を先に取ってから新しい札を置く**
  （先に置くなら約 2 秒以内に前の札を取る）。
- **turn と river が逆にならない**（ADR-0058 追記 4）。店舗の実卓で、turn を差し直したあと river を置くと逆に
  記録された（turn の札がリーダーの境目で載ったまま 15 秒読めず、river がその差し直しとみなされた）。そのハンドで
  一度でも読めなくなって戻った札は差し直しの対象にしない（本当に差し直していれば、次の札を置いた時点で詰め直す）。
  差し替えた札が戻ってきて、差し替えた札（最後に配った札）も載っていれば差し替えを取り消し、元の並びに戻す。
  ログの「見えていない札」に「読み直し N 回」を付ける。**turn の置き場所はリーダーの境目を避ける**（`docs/usage.md`
  「置く位置」）。
- **river のあとで turn を差し直しても turn と river が逆にならない**（ADR-0058 追記 5）。5 枚埋まったボードで 1 枚を
  取り除いて新しい札を置くと、新しい札を 5 枚目に入れて前を詰めていたため、river が 4 枚目になっていた。取ってすぐ
  置くと「5 枚を超えました」で位置なしのまま確定していた。新しい札は取り除いた札の位置に入れ、取り除いた札が約 6 秒
  見えなくなるまで「差し直し確認中」で待つ。店舗の卓の置き方（flop と turn の間を空け、turn を 3 台目のリーダーの上に）
  を `docs/usage.md` に記載。
- **卓モニタに「外れた N s」**。ボードに記録した札がいま読めていなければ薄く表示する（記録は変えない）。外したのに
  出なければ、その札はまだどこかで読めている。ログには、ボードの札を入れた位置・読んだリーダー（左から N 台目）・
  そのとき見えていなかった札を 1 行で出す（差し直しが次の位置に入ったときの切り分け用）。
- **卓モニタに「確認中」**（ADR-0058 追記 3）。読めているがまだ数えていないボードの札を点線で出す（「確認中 N s」/
  「差し直し確認中」）。置いたのに出なければ、その札はどのリーダーにも読めていない。数える前の札は 3 秒までの途切れを
  許す（リーダーの境目・重ね置きで途切れながら読める札の反映が遅れていた）。見え始め・読み直しを INFO ログに出す。
- 持ち上げて見る・一瞬の読み落ちでは差し替わらない（ISSUE-0026 の「同じ札が 2 か所」は再発しない）。明示の
  `cb` / `cs` もそのまま使える。
- `config.json` にキーが無くても既定で有効。`release_sec: null` で自動の差し替えを止め、`commit_sec: 0` で従来の
  「最初に見えた瞬間に確定」に戻せる。`redeal_window_sec: null` でボードの 1 枚だけの差し直しを止める。
  `tools/probe_pcsc.py` の検査は従来の解釈のまま。
- 記録: `RFIDEvent.replaces`（差し替えた前の札）を additive 追加し、`reconstruction_event` schema を `0.4` に。
  契約 `rfid-usb-ccid.md` v1.5〜v1.10 §4（firmware の要求は不変）。
...
- `update.cmd` / `-Update` が、差し替え前の古いスクリプトのまま続きの手順を実行していた → 更新後の
  インストーラで起動し直す。
- `update.cmd` が既定の `verify-v1` を取りに行き、別ブランチで入れた店舗 PC を古い版で上書きし得た →
  取得元のブランチを `installer\branch.txt` に覚え、`-Branch` 無しの更新はそれを使う（1 行インストールも記録する）。
...
記録済みのアクション履歴を系統的に壊していた。

- **ハンドごとにボタンを 1 つ回す**（pokerkit backend）。ボタンの次の席を先頭にした並びで
  pokerkit に渡すので、ブラインドを出す席と最初に行動する席が正しく移動する。
- **1 ハンド目のボタン席を訊く**（`--cli` / GUI のセッション設定。空 Enter = 最大の席番号 =
  **これまでと同じ並び**なので、既存の記録・golden fixtures の 1 ハンド目は変わらない）。
- **ポジション名で読み上げても通る**: 「BTN、コール」「ビッグブラインド チェック」が
  「シート3、コール」と同じ明示証拠として actor を決める（仕様 §7）。席番号があればそちらが優先。
  卓に無いポジション名は無視する（手番は動かない）。
- **記録に追加**: `HandSummary.button_seat` / `position_map`、`ActionRecord.position`
  （schema `hand` `1.1`→`1.2` / `action` `1.1`→`1.2`、いずれも optional の additive）。
- **卓状態モニタにボタンとポジション名を表示**（実機で回転を目視確認できる）。
- **legacy backend は変わらない**（ボタンを持たない単純ラウンドロビンのまま = rollback path 不変）。
- 回帰 `tests/test_positions.py`(54) / `tests/test_position_evidence.py` を追加。976 passed。

### Fixed (RFID のログが CLI の入力に割り込みコマンドが通らない, ISSUE-0034, 2026-09-12)

実機テストで **`w 1` を 3 回打って全部弾かれ、ウィナーを宣言できずハンドが確定しなかった**。

- **同じ位置・同じ札のボード再検出を DEBUG に落とした**（結合の弱いリーダーは載っている札を
  何度も読み直す。位置は固定なので記録上は無害だが、端末が埋まって入力が壊れる）。
- **ストリート自動遷移は実際に動いたときだけ INFO**。pokerkit backend では `advance_street` が
  no-op なので「既にそのストリート」の早期 return に入らず、**毎回「自動遷移した」と嘘のログ**を
  出し続けていた。動かないときは DEBUG で「backend keeps …」と出す。
- **`w1` / `r1 500` を受け付ける**（空白の打ち損ねを許容。`q`/`n` と読み上げ文は対象外）。
...
- `_resolve_actor` が **席を問わず ±2 秒で最近傍の RFID seat 検出**を明示発話席より優先し、
  ...
  持ち上げた札を置き直しても 1 件出るため、**行動と区別できない**。
  カメラ（chip motion = 行動の観測）を廃止した結果、存在検出が「物理証拠」の座に繰り上がっていた。
- **RFID を actor 証拠から外した**。sensed は **明示発話席のみ**。RFID は
  `_pop_matching_rfid_event`（**同席限定**）の裏付けとしてのみ使う（actor を別席へ動かす力が無い）。
- golden `out-of-turn-rfid` は不具合を正解として固定していたので、
  **`rfid-appear-is-not-an-action` へ改名して期待値を作り直した**（配布 → 席の言及が無い「コール」→
...
- hand logger が `logs/{session}.table_state.json` に卓状態を publish する（config
  `table_state.enabled` 既定 **true**）。**カードが外れても RFIDEvent は出ない**ため、engine が
...
- 要件定義書 `sprc_v4.docx` は当初からアクター推定を**尤度ベース**と規定していた（FR-26/27・§5.4・
  FR-25/35）。現在の決定的なライブ確定は ISSUE-0009 の暫定が定着した **drift**（ISSUE-0031）。
- **決定**: 記録の正本を**事後推定**に移し、ライブ経路は **provisional 表示**に降格する。読み取りは
  **live ⊕ estimate ⊕ staff corrections**。推定器は **pokerkit の合法手列に対する制約付きビーム探索**
  （決定的 = replay 決定性を保つ）+ 観測尤度（ASR n-best / 席・ポジション言及 / **RFID プレゼンスの
  非対称な扱い** / デッキ整合 / 終端制約）。`SILENT_FOLD_CAP=2` は幾何減衰するサイレント事前確率に置換。
- **Fable 5.1 の設計監査**を受け、中核 3 件を実測検証した。うち 2 件は **P0 の欠陥**:
  - **ISSUE-0033**: RFID の「カード検出」が actor 証拠として明示発話席より優先されており、
    **ホールカードを配っただけで誤 silent fold が起きる**（golden `out-of-turn-rfid` がその挙動を
    正解として固定している）。オーナーが唯一リアルタイム精度を要求した fold を壊す。
  - **ISSUE-0032**: **ディーラーボタンが回らない**（6-handed で 3 ハンド連続、最初の actor は毎回
...
- **合成 silent-fold の時刻**が「次の人が行動した時刻」になっていた（ディーラーが宣言しない fold は
  後続アクションで初めて推定されるため）。RFID が席ごとに「**カードが消えたまま戻っていない時刻**」を
  保持し、その時刻を fold の `timestamp` に使うようにした。
- **不在を fold の判定には使わない**。プレイヤーはカードを持ち上げて見るので不在 ≠ fold。判定は
  従来どおり合法手・actor 推定が行い、**時刻だけ**を差し替える。ハンド範囲外の観測は棄却し、
  新ハンド / ミスディール訂正で観測を捨てる。`source.rfid` で時刻の出所が分かる。
  **confidence は据え置き**（判定材料にしていないので較正値を動かさない）。
- **ボード各枚の配布時刻**を `HandSummary.board_timeline`（`[{index, card, dealt_at}]`）に追加。
  時刻は**最初の検出**で固定する（結合の弱いリーダーは同じ札を何度も再検出するため）。
  ターン = 4 枚目 / リバー = 5 枚目がラウンドの区切り、フロップは 3 枚の最小値が開始
  （フロップ内の順序は意味を持たない）。訂正で位置を取り消せば時刻も採り直す。
- `hand` schema `1.0` → `1.1`（optional フィールド追加 = additive）。881 passed。
...
- **一度読ませたカードを外して正しいカードを読ませ直す**（ミスディール）操作に対応した。従来は
  ...
  （ストリートが誤って進む）、席のホールカードは 2 枚上限で **差し替えが黙って無視**されていた。
- **`cb <位置>`**（`--cli`）= ボードの N 枚目の記録を取り消す。**他の位置は動かない**ので
  真ん中の 1 枚だけ差し替えてもボードの順序が壊れない。取り消した札を外して正しい札を置けば
  同じ位置に入る。
- **`cs <席>`**（`--cli`）= その席のホールカード記録を捨て、載っている札を読み直す（2 枚上限に
  引っかからない）。
- **iPad / staff API からも同じ操作ができる**（control queue に `correct_board`(`index`) /
  `correct_seat`(`seat`) を additive 追加, ADR-0039 の経路。`ViewerApiClient.send_control(..., index=)`）。
- **absence からの自動判定は入れない**（カードが見えなくなったことを載せ替えと解釈しない）。
  実機で数秒の読み落ちを観測しており、自動解放は ISSUE-0025/0026 の壊れ方（同じ札が 2 か所 /
  枚数の水増し）を再発させる。ミスディールはディーラーが宣言する事象なので明示コマンドにした。
- 訂正は「記録を捨てて**物理的に載っているものを読み直す**」と定義し、RFID のデバウンスも落とす。
  **冪等**なので、札を外す前に打ってしまっても外して打ち直せば収束する（不可視の状態を持たない）。
- 訂正が入ったハンドは **`needs_review`**（監査痕）。ストリートは戻さない（1 枚差し替えても
  「フロップはフロップ」）。**音声語彙には追加していない**（「訂正」は一般語で、誤爆すると
  正しい記録を消す方向に働くため。staff の明示操作のみ）。
- 契約 `docs/contracts/rfid-usb-ccid.md` を **v1.4**（§4 に解放の 2 経路を明記。**firmware 無改修**）。
  869 passed。

### Fixed (全角の CLI コマンド `ｎ` が認識されない, ISSUE-0030, 2026-09-12)

- `--cli` は読み上げ文と同じ入力欄でコマンドを打つので、日本語 IME のままだと `ｎ` になって
  弾かれていた（金額は `parse_amount` が全角数字を解釈するので、通らないのはコマンド letter だけ）。
- **コマンド照合用にだけ**全角 ASCII と全角スペースを半角へ寄せた（`main._normalize_cli_command`）。
  読み上げ文は **正規化前の行**を `parse_action` に渡すので `raw_text` は入力そのまま。
  `ｑ` / `Ｎ` / `ｗ　１` / `ｒ　１　５００` も通る。ISSUE-0027 と同じ「入力側を正規化する」方針。
...
- **照合の前にひらがな→カタカナへ正規化**した（`audio/recognizer.py:_to_katakana`）。キーワード表を
  増やさないので語彙の二重管理が起きない。`U+3041..U+3096` を `+0x60` する **1:1 写像で文字位置が
  保たれる**ため、`parse_action` の「最左キーワードを採用」規則が崩れない。
- 席表現（`シート3` / `しーと3`）にも適用。金額（数字・漢数字）は対象外。
  **`raw_text` は正規化前のまま**（ログで実際の入力が分かるように）。
- **ASR にも同じ経路で効く**（Whisper が「ちぇっく」と書き起こしても拾える）。835 passed。
