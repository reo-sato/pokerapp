...
gui / staff app / viewer API / integration（ADR-0035 / ADR-0036）

## Expected Behavior

ADR-0035（staff iPad アプリ）/ ADR-0036（staff API 拡張）に沿って、店舗操作を 1 つのタッチアプリで
...
3. **認可運用（shared token の配布・回転）**: staff token は端末に保存（ADR-0035 §3）。複数 iPad
...
5. **desktop GUI と staff app の責務分界の最終形**: 当面並存（ADR-0035）だが、将来 desktop を
...
- Q2（session 結線）: ADR-0036 §B 実装時に hand logger 側の「既存 session 選択」運用を併せて設計。
...
- `docs/adr/0035-staff-ipad-app-touch-frontend.md`
- `docs/adr/0036-staff-api-session-seat-handlogger-expansion.md`
...
プロセス所有境界に触れる open question。実装は会計/注文（API 済）→ session/座席（ADR-0036 §A/B）→
