# Issue 0014: PN5180 + ESP32-S3 firmware ↔ Python の HTTP API 契約固定
...
**Superseded by ISSUE-0015**（2026-06-01）。本 issue は ADR-0014（Superseded）の前提
（HTTP transport が canonical）で起こした。ADR-0015 で **PC/SC 経路が canonical** に方針変更
されたため、firmware ↔ host の契約対象が HTTP API から USB CCID / PC/SC に移動。後続の
open question は ISSUE-0015（USB CCID firmware 契約）で追跡する。本 issue は history として残す。
...
ADR-0014 の方針に従い、ESP32-S3 firmware と Python `RFIDHTTPReceiver` の間の **HTTP API 契約が
固定** されている:
...
ハードウェア仕様変更（ADR-0014）で reader IC が世代交代したが、firmware と Python の **API 契約**
...
- `docs/adr/0014-migrate-rfid-to-pn5180-esp32s3-and-canonical-http-transport.md`
