# Issue 0005: 並行開発における contract drift / 凍結タイミングの blocking risk
...
3. **mock と実 repository の乖離**: mobile の mock が契約 fixtures から逸脱し、実 repository
   差し替え時に UI 層が壊れる。
4. **ISSUE-0003 依存**: S3 の残高計算 API 契約は point 残高 source of truth（ISSUE-0003）が
...
2. その後 WS1 が `display_name` 正規化ポリシー（ISSUE-0004）を変更 → 契約が drift。
...
- ISSUE-0003 を S3 着手の gate にする（残高契約凍結まで ledger WS を本実装に入れない）。
...
- ISSUE-0003（point 残高 source of truth）は S3 ledger WS の直接 blocker。
- ISSUE-0004（display_name uniqueness）は player 契約変更時の drift 例として参照。
