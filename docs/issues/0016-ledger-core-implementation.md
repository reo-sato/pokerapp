...
ADR-0016 の方針に沿って S3.1 core を実装した:
...
残（freeze 前 / 後続）: idempotency_key の運用詳細・speculative の UI 表現（S3.2, ISSUE-0017）、
ledger schema の `1.0` freeze（session freeze #3 後）。settlement の CSV export は S3.3（ISSUE-0018）。
...
ADR-0016 / `docs/contracts/ledger-overview.md` / `ledger-schema.md` の契約に対し、ledger / points の
...
S3 planning（ADR-0016）で契約方針のみ確定。実装は本 issue（S3.1）で行う。
...
- `docs/adr/0016-ledger-points-settlement-design-direction.md`
...
- ISSUE-0001 の残サブ問題（idempotency_key 運用、speculative 表示）を本 issue / ISSUE-0017 で具体化する。
