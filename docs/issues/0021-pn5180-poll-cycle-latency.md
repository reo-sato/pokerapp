...
**Fixed（実機確認済, 2026-09-11 `83ad762`）** — 11 台満載で 1 周 **272/297〜301/331 ms**
（目標 ≤ 300 ms を平均で達成、hard bound 0.5 s に 34% の余裕）。経緯は下の「実機フィードバック 1〜8」。
...
## 実機フィードバック 7（2026-09-11, `047ee1e` = 狙い撃ち導入後を 10 台 ready + 満載で実行）

```
poll 統計(直近 19 周): 1 周 min/avg/max = 424/433/434 ms, 最長 reader #3 = 46 ms, probe 最大 3,
           狙い撃ち 340/342 命中, 応答待ち最長 7.4 ms, coll_pos fallback 0, ノイズ再試行 0
```

**538 → 433 ms**。狙い撃ちは意図どおり効いている:

- **狙い撃ち 340/342 命中（99.4%）** = 18 枚 × 19 周ぶんがほぼ全部 1 probe で確定。
- **`coll_pos fallback` が 255 → 0**（DFS に落ちなくなった）。**probe 最大 6 → 3**。
- **`応答待ち最長 7.4 ms`** が取れた。32 bit mask の見積り（tx 2.72 + t1 0.32 + rx 3.63 = 6.66 ms）に
  IRQ ポーリング粒度（100 µs）とレジスタ読みを足した値で、**フレーム長連動の上限（tx+8 = 10.7 ms）が
  正しかった**ことの裏取りになった（固定 8 ms のままなら 64 bit mask は打ち切っていた）。

### 残り時間の内訳（reader 1 台・2 枚。実測 43〜46 ms）

| 項目 | 回数 | 1 回 | 小計 |
|---|---|---|---|
| 狙い撃ち probe | 2 | 7.4 ms | 14.8 ms |
| Stay Quiet | 2 | ≈4.2 ms | 8.4 ms |
| root probe（空 = 上限いっぱい） | 1 | ≈9.5 ms | 9.5 ms |
| RF on/settle/off + MUX ほか | — | — | ≈2〜10 ms |

→ **Stay Quiet と root probe で 18 ms（4 割）**を使っている。10 台・18 枚だと
Stay Quiet 76 ms + root 95 ms = **171 ms/周**。

### 対処（簡略サイクル = Stay Quiet と root probe を間引く）

Stay Quiet は **root probe で「新しい札」だけを見るための下準備**なので、root を送らないサイクルでは
不要。そこで **狙い撃ちが当たる reader では `PN5180_FAST_CONFIRM_EVERY` 回に (N-1) 回を
「狙い撃ちだけで終える簡略サイクル」** にした（`PN5180_FAST_CONFIRM_EVERY` を 5 → **3**）。

- 簡略サイクル（2 枚）: 狙い撃ち 2 のみ ≈ 17 ms（完全確認の半分以下）。
- 見積り: 簡略 ≈ 262 ms / 完全確認 ≈ 433 ms → **平均 ≈ 320 ms**（N=3）。N=5 なら平均 ≈ 296 ms。
- **代償**: 既に札がある reader に**増えた**札の発見が最大 N poll 遅れる。
  **空の reader（前回 0 枚）は簡略に入らない**ので、席の 1 枚目 / flop / turn / river は毎 poll 検出。
  遅れるのは「席の 2 枚目」だけで、ディーラーが 2 周目を配る間隔より十分速い。
- 併せて `CARD_POLL_INTERVAL_MS` を 100 → **50**（1 周が縮んだぶんを検出遅れの短縮に回す。
  `poll 統計` の「1 周」はスキャン時間だけで、検出遅れの上限は 1 周 + この値）。
- 統計に **`簡略 N/M reader周`** を追加（分母 = 窓内の 周数 × ready reader 数）。

## 実機フィードバック 8（2026-09-11, `081c9bb` = 簡略サイクル導入後を **11 台 ready** + 満載で実行）

配線振替で 11/11 が揃った状態での初測定（`配線 OK: 設定 11 ch すべて通電` / `PN5180 ready: 11/11 reader`）。

```
poll 統計(直近 27 周): 1 周 min/avg/max = 238/324/495 ms, 最長 reader #7 (index 6) = 60 ms,
     probe 最大 4 回/reader, 狙い撃ち 533/540 命中, 簡略 180/297 reader周,
     応答待ち最長 7.4 ms, coll_pos fallback 0, ノイズ再試行 3, ready 11 reader
```

- 簡略サイクルは効いている（`簡略 180/297` = 60.6%、狙い撃ち命中 98.7%、`coll_pos fallback` 0）。
- **平均 324 ms は見積り（320 ms）どおり**だが、**max 495 ms** が残った。

### Root Cause 7: 簡略サイクルが全 reader で lockstep していた

`s_confirm_skips[slot]` は「連続スキップ回数」で、完全確認をした周に 0 に畳む実装だった。
**ハンドの切れ目で全 reader が 0 枚になる**（= 簡略の条件 `prev->count > 0` を外れて一斉に 0 に畳まれる）
ため、次のハンドで札が載ると **11 台が同じ周で完全確認に入る**。つまり 1 周は
「全部簡略（238 ms）」か「全部完全確認（495 ms）」のどちらかに振れる。

実測 3 データ点が 1 台あたりの内訳を一意に決める（11 台で割るだけ）:

| | 1 台 | 11 台 |
|---|---|---|
| 簡略サイクル | 21.6 ms | **238 ms**（= 実測 min） |
| 完全確認サイクル | 45.0 ms | **495 ms**（= 実測 max） |
| 差（Stay Quiet + root probe） | 23.4 ms | 257 ms |

検算: N=3 の lockstep なら 3 周に 1 回が完全確認 → `(2×238 + 495)/3 = 323.7 ms` = **実測 avg 324**。
初期値だけをずらしても毎ハンド再同期するので、**位相は自由走行カウンタで決める**必要がある。

### 対処（位相ずらし + `CONFIRM_EVERY` 3 → 6）

完全確認を行うのは **`(poll 周回 + reader index) % CONFIRM_EVERY == 0` の reader だけ**にした
（poll 1 周の末尾で +1 する `s_poll_cycle` が唯一の位相源。0 枚を跨いでも崩れず、周期はどの
reader も厳密に `CONFIRM_EVERY`）。1 周に完全確認するのは `ceil(11/N)` 台だけになる。

上の 21.6 / 45.0 ms から予測される 1 周（11 台満載）:

| N | 1 周に完全確認 | 1 周 min〜max | avg | 増えた札の発見遅れ |
|---|---|---|---|---|
| 3 | 3〜4 台 | 308〜331 ms | 323 ms | ≈ 1.0 s |
| 4 | 2〜3 台 | 284〜308 ms | 302 ms | ≈ 1.2 s |
| 5 | 2〜3 台 | 284〜308 ms | 289 ms | ≈ 1.4 s |
| **6** | **1〜2 台** | **261〜284 ms** | **281 ms** | **≈ 1.7 s** |

**N=6 が「max が初めて 300 ms 未満」になる最小値**なので `PN5180_FAST_CONFIRM_EVERY` を 3 → **6**。
N を 7 以上にしても `ceil(11/6)=2` から減らないので max は縮まない（avg だけ僅かに下がる）。
遅れが気になるなら 3 に戻す（max 331 ms）。遅れるのは「**既に札がある** reader に増えた札」＝
席の 2 枚目 / flop の 2・3 枚目だけで、**空の reader は毎周完全確認**（席の 1 枚目 / flop の 1 枚目 /
turn / river）。札の**離脱**は狙い撃ちが毎周走るので N に影響されない（`PRESENCE_HOLD_MISSES=3` のまま）。

### 実機結果（2026-09-11, `83ad762` = 位相ずらし + N=6 を 11 台 ready + 満載で実行）— **目標達成**

```
（札を置いた直後の過渡）1 周 min/avg/max = 276/311/720 ms, probe 最大 8, coll_pos fallback 16
（定常 1）              1 周 min/avg/max = 275/297/311 ms, probe 最大 4, 狙い撃ち 628/638, 簡略 266/319
（定常 2）              1 周 min/avg/max = 272/301/331 ms, probe 最大 4, 狙い撃ち 621/638, 簡略 266/319
```

| | 位相ずらし前（`081c9bb`） | 位相ずらし後（`83ad762`） |
|---|---|---|
| 1 周 min〜max | 238〜**495** ms | 272〜**331** ms |
| min/max の差 | **257 ms**（lockstep） | **36〜59 ms**（1〜2 台ぶん） |
| avg | 324 ms | **297〜301 ms** |

- **`簡略 266/319` = 1 周あたり完全確認 1.83 台**（319 = 29 周 × 11 台 → full 53 件 ÷ 29 = 1.83 = 11/6）。
  位相の配分が設計どおり `ceil(11/6)=2` 台以下に収まっている。
- **目標 ≤ 300 ms は平均で達成**（297〜301 ms）、max 331 ms は hard bound 0.5 s に対して 34% の余裕。
- 予測（261〜284 ms）との差 ≈ +11〜47 ms は **`ノイズ再試行 8〜14` と狙い撃ちの外れ 10〜17 件**
  （各 ≈ 10 ms の RX 上限待ち = 0.9 件/周 ≈ 9 ms）で説明がつく。
- 過渡の `max 720 ms / probe 最大 8 / coll_pos fallback 16` は**札を置いた瞬間だけ**（root probe + DFS で
  衝突を解いている）。定常では `fallback 0 / probe 最大 4` に落ちる。

## Regression Check

**実機で** `idf.py monitor` の `poll 統計` を見る（自動テスト不可 = 実 RF が要る）:

- **11 台満載で 1 周 min/max の差が ≤ 2 台ぶん（≈ 50 ms）**（= 位相ずらしが効いている。
  差が 250 ms 以上開くなら lockstep に戻っている）。**2026-09-11 実機で 36〜59 ms を確認**。
- **`簡略 M/(周数 × ready)` が `1 - ceil(11/N)/11` 付近**（N=6 なら ≈ 0.83 = 266/319）。
...
0/1/2/3/5 枚・ノイズ・debounce を確認済み。**位相ずらし（実機フィードバック 8）も simulator で
検証できる**（実 RF 不要）: 11 台に 1 枚ずつ載せて `CONFIRM_EVERY` 周まわし、
①1 周の完全確認が `ceil(11/N)` 台以下 ②`CONFIRM_EVERY` 周で全 reader がちょうど 1 回ずつ完全確認
（= 周期が厳密に N）を確認する。probe 数で見分ける（1 枚の reader は 簡略 1 probe / 完全確認 2 probe）。
...
5. **`PN5180_SPI_HZ` 1 MHz → 5 MHz は行わない（決定, 2026-09-11）**: `app_config.h` のコメントは
   「11 台が 1 MHz で安定したら上げる」だったが、**上げない**ことにした。効果は 1 周あたり
   ≈ 10〜20 ms（3〜7%）と小さく、一方この配線は **13 コネクタ中 2 本で BUSY 導通不良が出た**
   （ISSUE-0023）= ハーネスの信号品質に実績のある弱さがある。5 倍のクロックで間欠的な SPI 化けを
   招くと「本番中にランダムに読めない」という最悪の形で出る。1 MHz 据え置きで目標は達成済み。
   将来 1 周をさらに詰める必要が出たら、先に **`PN5180_FAST_RX_TIMEOUT_MS` 8 → 6**
   （実測 `応答待ち最長 7.4 ms` − 送信 2.7 ms = 応答に使っているのは 4.7 ms なので 6 ms でも
   余裕 28%。外し過ぎは `狙い撃ち H/P` の命中率低下で分かる）か **`PN5180_FAST_CONFIRM_EVERY` を上げる**
   （検出遅れとの交換）を検討する。いずれも数 % の改善なので、実運用で困るまで手を付けない。
6. **重ね置き 3 枚が電力不足で応答しない可能性（firmware では解決できない）**: Stay Quiet を
