...
| ADR-0027 | L1 per-player PIN 認証の詳細設計（player principal 解決レイヤ） | Proposed | 2026-06-14 | identity / auth (L1, 設計) | `docs/adr/0027-l1-per-player-pin-auth-design.md` | PIN を node-local `player_credentials.json`（PBKDF2 + lockout、players.json/sync 非対象）に分離（ADR-0025 素描を精緻化）。検証後に stateless 署名トークン発行 → principal 解決レイヤで player self-write を認可。config `viewer_api.player_auth` 既定 off で後方互換。**設計のみ・コードなし**。関連: ADR-0025/0021/0004/ISSUE-0019 |
