...
| ADR-0030 | player merge の設計（alias / tombstone・read-time canonicalization） | Proposed | 2026-06-14 | identity / registry (L2 前提) | `docs/adr/0030-player-merge-alias-design.md` | merge = player レコードに `merged_into` を付ける **alias/tombstone**（履歴 rewrite なし→append-only・収束 sync・player_id 不変を保つ・可逆）。`resolve_canonical` で全 player-keyed read（settlement 集計 / viewer / seating / order / login principal）を survivor に解決。survivor はスタッフ明示。schema `1.0`→`1.1` additive。staff API + GUI。**設計のみ・コードなし**。関連: ADR-0028/0029/0004/0016/0022/0019 |
