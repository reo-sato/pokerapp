# ADR-0036: staff API 拡張 — session/座席ライフサイクルと hand logger 遠隔制御
...
ADR-0035 で店舗操作を staff iPad アプリ（staff API 上のタッチ front-end）に統合する方針を確定した。
...
関連: ADR-0035（staff app）/ ADR-0021（staff token / 単一書き手 / RLock）/ ADR-0018（in-process API）/
...
**v1 では read-only**（`GET .../hands` で履歴閲覧のみ。ADR-0035 §5）。
...
いずれも **録音主体は PC**（ADR-0035 §4）を崩さない。hand logger 状態の二重書き込み（race）を
...
- Supersedes: —（ADR-0035 の不足 API を具体化。ADR-0021 の staff write を additive 拡張。
