...
並行開発の主リスクである **contract drift**（ISSUE-0005）を実際に防ぐには、契約の物理的な
...
- `docs/issues/0005-parallel-dev-contract-drift.md`（drift risk）
- `docs/issues/0006-hand-id-int-vs-cross-app-string.md`（hand_id の cross-app 形）
...
- **Alternative A — 契約を各 model の ADR 本文に散在させる（専用ディレクトリなし）**
  ...
  - Why rejected: drift detection（ISSUE-0005 の核心）が成立しない。
...
- Neutral / new constraints
  ...
  - `session_id` / `hand_id` の cross-app 形は未確定のまま（ISSUE-0006）。
...
- [ ] `session_id` / `hand_id` の cross-app 形を S2 で確定（ISSUE-0006）。
