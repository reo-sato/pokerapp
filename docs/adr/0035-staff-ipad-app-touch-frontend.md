# ADR-0035: 店舗用 staff iPad アプリ — staff API 上のタッチ統合 front-end
...
（不足する staff API の追加設計は ADR-0036 が担う）。
...
ウィナー / リバイ等）を遠隔送信する**だけにする。この遠隔制御に必要な staff API は **ADR-0036** が
設計する（現状 hand logger 制御 API は存在しないため、ハンドロガータブは ADR-0036 完了まで
read-only から始める）。
...
├─ SessionList      … session 一覧 + 新規作成/close          [一部 要 ADR-0036]
└─ TableView(session)  … 卓ごとに以下をタブで統合
    ...
    ├─ 座席 (Seating)  … seat→player 割当 / player 登録・選択                          [要 ADR-0036]
    └─ ハンド (Hands)  … hand 履歴 read ✅ / 新ハンド・ウィナー等の遠隔操作              [要 ADR-0036]
```
...
| **SessionList** | session 一覧（label / status / 卓状況）、「新規セッション」 | session 選択 → TableView / session 作成 / close | read: `GET /api/players` 他 / **新規**: session create・close（ADR-0036） |
| **TableView → 会計** | player 選択、種別[buy_in/...]・cash・point・メモ、buy-in プリセットボタン、ポイント付与、中間集計、精算確定、支払状態(paid/unpaid/partial)+受領額 | エントリ追加 / 取消(reversal) / ポイント付与 / 精算 commit / 支払額記録 | 既存: `POST .../ledger-entries`, `.../settlement/commit`, `PUT .../payment(-status)`, `GET .../settlement`, `GET /api/staff/buyin-presets`。reversal/grant は **新規**（ADR-0036） |
...
| **TableView → 座席** | 席ごとの player 割当、player 新規登録、carry-forward | seat→player 設定 / player 作成 / リネーム / merge | **新規**: assign_seat / player create・rename（ADR-0036）。merge は既存 `POST /api/staff/players/merge` |
| **TableView → ハンド** | hand 履歴（actions / board / pot / needs_review）。将来: 新ハンド/ウィナー/リバイ操作 | read（v1）→ 遠隔制御（後続） | read: 既存 `GET .../hands` / `.../hands/{hid}`。制御は **新規**（ADR-0036） |
...
- Negative / trade-offs: 新 front-end の保守対象が 1 つ増える（`mobile/` と二本立て）。座席管理・
  ハンドロガー遠隔制御は **staff API の追加（ADR-0036）が前提**で、それまで該当タブは限定機能。
...
- `api/server.py`（既存 `/api/staff/...` を再利用 + ADR-0036 で追加）
- `mobile/`（型・repository パターンの参照元、改変しない）
- `docs/contracts/viewer-api.md`（staff 節 / 追加は ADR-0036）
...
- Supersedes: —（関連: ADR-0017 / ADR-0018 / ADR-0020 / ADR-0021 / ADR-0026。不足 API = ADR-0036、
