# ADR-0008: PN5180 + ESP32-S3 via USB CCID — PC/SC is the canonical RFID transport
...
ADR-0007 で「HTTP transport を canonical、PC/SC は legacy」と決めたが、これは作業者
...
これにより ADR-0007 は前提が誤りだったため Superseded に倒し、本 ADR を Accepted で起こす。
...
`config_default.json`, ADR-0007（Superseded）。
...
3. **`reader_configs` の reader 名 ↔ 役割マッピング契約を維持**: `[{"name": "...", "role": "seat",
   ...
   CCID slot name に追従する（firmware 側で固定する → ISSUE-0007 で追跡）。
...
6. **ADR-0007 は Superseded**。本 ADR が同一の対象（PN5180 + ESP32-S3 移行）に対する正しい判断。
   ADR-0007 を削除はせず、history として残す（CLAUDE.md docs-as-code 流儀）。
7. **firmware ↔ host の契約固定は ISSUE-0007 に集約**: USB CCID descriptor / vendor strings /
   reader_name / slot 配置 / ATR（ISO 15693 用 proxy ATR）/ pseudo-APDU セット / card
   inserted-removed 通知。ISSUE-0006（HTTP 契約）は本 ADR で Superseded by ISSUE-0007 に倒す。
...
- **Alternative D（ADR-0007 採用案）— HTTP を canonical、PCSC を legacy**
  - Why rejected: PC/SC 経路は本筋であり、HTTP は本筋ではない（user 方針）。本 ADR で
    ADR-0007 を Superseded に倒す。
...
- Neutral / new constraints
  - reader_name 文字列が firmware の CCID slot name に依存する（contract 化要 → ISSUE-0007）。
  - tag UID 長 4/7/8B 許容は ADR-0007 から引き継ぐ（PCSCBridge / card_master 側の確認 follow-up）。

## Validation / Follow-up

- [x] ADR-0007 を Superseded に更新し、本 ADR で Superseded by を相互リンク。
- [x] ISSUE-0006 を Superseded by ISSUE-0007 に倒し、本 ADR の方針に追従する新 issue（ISSUE-0007,
...
- [x] decision-log.md に ADR-0008 / ISSUE-0007 を追加、ADR-0007 / ISSUE-0006 を Superseded に。
...
- [ ] ESP32-S3 firmware の USB CCID descriptor / reader_name / ATR を ISSUE-0007 に固定。
...
- **Supersedes: ADR-0007**（HTTP canonical の判断は誤りだった）
- Superseded by: —
- 関連: ISSUE-0007（USB CCID firmware 契約）、ISSUE-0006（本 ADR で Superseded）
