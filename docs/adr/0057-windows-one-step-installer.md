...
- **取得元のブランチを覚える**（`installer\branch.txt`, git 管理外）。`update.cmd` は `-Branch` を渡さないため、
  覚えていないと既定の `verify-v1` で別ブランチの導入を古い版に上書きする。`-Branch` 指定時と 1 行インストール
  （bootstrap）が記録し、`-Branch` 無しの実行はそれを使う。
