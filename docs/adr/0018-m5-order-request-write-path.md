# ADR-0018: M5 — 注文リクエスト write path（staff-in-the-loop / in-process API / menu master / name-pick 確定）
...
M5（ADR-0017 ロードマップ）はプレイヤーのスマホからのドリンク注文 = **viewer API 初の write 系**。
前提として保留していた事項を確定する必要がある:

1. **ISSUE-0019（プライバシーモデル）**: 無認証 name-pick のまま write を許すか。
2. **注文と台帳の整合**: player の入力をそのまま `ledger_entry` にするか、スタッフ確定を挟むか
   （ADR-0017 / verify-v1 ledger（ADR-0016）で staff-in-the-loop を既定案として記録済）。
...
1. **本人確認は name-pick のまま**（user 決定, 2026-06-11。ISSUE-0019 を v1 として決着）。
...
2. **staff-in-the-loop を採用**: player は `order_request`（pending）を POST するだけで、
   **ledger には書かれない**。スタッフが `--ledger` 画面で確定（confirm）したときに初めて
   ...
   append-only の台帳整合・価格決定権をスタッフ側に残す（verify-v1 ledger = ADR-0016, `gui/ledger_view.py`）。
...
6. **closed session への確定ガードは order-request 層が担う**: verify-v1 ledger（ADR-0016）の
   `add_entry` は closed session を拒否しないため、`confirm_request` が確定前に session 状態を
   見て closed なら `session_closed`（409）で弾く（注文の closed 不変条件を front-end に依存させない）。

## Alternatives Considered

- **player 直接 ledger 書き込み** — スタッフ不在でも台帳が増え、価格・重複・いたずらの検証点が
  ない。却下（verify-v1 ledger の append-only 方針, ADR-0016 とも整合）。
...
- Positive: viewer API に write が入っても台帳は append-only + スタッフ検証を維持。
  ISSUE-0019 が v1 として決着し M5 の gate が外れる。menu master で価格整合。
...
- [ ] 将来: PIN の再評価（問題が顕在化した場合）、order_request schema の freeze（ledger schema freeze と同時）

## Related Files

- `core/order_request.py` / `core/order_request_repository.py` / `core/menu.py` / `menu.json`
- `api/server.py` / `gui/ledger_view.py`（注文確定/却下パネル）/ `main.py` / `mobile/`
- `docs/contracts/{viewer-api.md,error-shapes.md,validation-rules.md,repository-interfaces.md}` /
...
- Supersedes: —（ADR-0017 §ロードマップ / verify-v1 ledger（ADR-0016）の append-only 方針を確定。ISSUE-0019 を v1 決着）
- Superseded by: —

> 統合メモ（2026-06-13）: 本 ADR は serene ブランチで ADR-0015 として起票されたが、
> verify-v1 統合時に PN5180 RFID の ADR-0014/0015 と番号衝突するため **ADR-0018 に採番替え**。
> ledger 参照は verify-v1 の cash+point/settlement 実装（ADR-0016）に読み替え、注文確定 UI は
> `gui/ledger_view.py`（serene の `gui/ledger_entry.py` は不採用）。
