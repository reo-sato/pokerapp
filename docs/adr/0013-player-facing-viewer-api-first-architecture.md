# ADR-0013: Player 向け参照は API-first（viewer API 前倒し）+ Expo（web export 先行）
...
- [ ] M4 以降: ledger（cash-only 先行凍結）→ 注文 write path → ISSUE-0001 決着 → settlement
