...
>
> **注 (ADR-0011)**: camera 入力は sprc_v4.docx 準拠で廃止済み。本 doc 内の camera 記述は
> historical で、現行の `reconstruction_event` は **audio / RFID の 2 ソース**（schema・recorder 反映済み）。

## 1. なぜ record/replay か

hand core の入力は **ノイジーな realtime センサー列**（マイク ASR ＋ RFID。camera は ADR-0011 で廃止）で、`IntegrationThread`
...
- **記録対象は audio / RFID の 2 ソース**: camera は ADR-0011 で廃止済み（旧設計では
  `CameraEvent.frame` を除外し seat/timestamp のみ記録する想定だったが、camera 自体を削除した）。
...
1. **timestamp は event 由来**。engine に **clock source を注入**する:
   ...
   - replay = **観測済み最大 event timestamp**。`MATCH_WINDOW` / `BUFFER_TTL`（`engine.py`）は
     event time 上で動かす（live/offline で同一挙動）。
2. **スレッド/キュー競合に依存しない**。replay は event を **timestamp 昇順**で同一 drain ロジック
   （`_drain_rfid_queue` 相当）へ通す。live のキュー順序は「概ね timestamp 順」で
