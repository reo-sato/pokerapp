...
ここで境界を定義する。重い識別子・抽象度の判断は ADR-0016 で確定済み。
...
- **ポイント残高 = この台帳の fold**: ある player の残高 = その player の `delta_points` 総和。
  cached カラムを権威にしない（**ISSUE-0001 → 選択肢 A 採用**, ADR-0016）。
- **cross-field**（schema 外, core が enforce）:
  - `delta_points != 0`。
...
- 結論: **金銭の自動再構築はしない / 手動入力 / seating からの roster 先読みのみ**。詳細・残課題は
  ISSUE-0007 と本 phase の ISSUE-0016〜0014。

## freeze 状態 / 未確定事項 / blockers

- **本 doc + `ledger-schema.md` + ADR-0016 + 実 schema/fixtures + core repository は実装済（S3.1, ISSUE-0016）**
...
- **決定済（ADR-0016）**: 2 台帳モデル（ledger_entry + point_ledger_entry）/ 整数円・整数点 /
