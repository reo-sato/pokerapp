...
>
> **注 (ADR-0011)**: camera 入力は sprc_v4.docx 準拠で廃止済み。本 doc 内の camera 記述
> （data flow / actor 融合の弱シグナル / source schema）は historical。現行入力は **RFID + 音声の 2 ソース**。
...
// action（= ActionRecord, additive）
{
  ...
  "properties": {
    ...
    "actor_source":   {"type":["string","null"],"enum":["turn","rfid","audio",null]},
