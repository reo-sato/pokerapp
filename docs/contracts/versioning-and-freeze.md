...
| 4 | ledger_entry / point_ledger_entry / session_settlement | S3 | **draft（ADR-0011）, schema/fixtures/core 実装済（S3.1, version 0.x, 未 freeze）**。ISSUE-0001 は残高=fold で決着。`schemas/*` + `fixtures/` + `core/ledger*.py` + tests 緑。freeze は session(#3) freeze 後 |
