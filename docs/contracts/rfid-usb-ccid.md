...
**version: 1.5 (ADR-0058。1.0 frozen 起点、以降は additive)** ／ canonical RFID transport（ADR-0015）の
...
- **卓の流れに合わせた解釈（v1.5, ADR-0058, 本番の起動経路で有効）**:
  ...
  - **配り直しは入力なしで反映する** **SHOULD**。位置を差し替えてよいのは、**そのストリート
    （flop 1-3 / turn 4 / river 5）の札が全部 `release_sec`（既定 6 秒）以上見えず、新しい札が
    `commit_sec` 載り続けた**ときだけ（一度差し替えが始まったストリートは、残りの消えた札も対象）。
    flop の 1 枚が読み落ちている間に turn が来ても、turn は 4 枚目のまま。席も同じく、前の札が
    `release_sec` 以上見えず新しい札が載り続けたら差し替え、同じハンドで別の席に記録した札は
    元の席から消えて新しい席に載り続けたら移す。差し替えは `RFIDEvent.replaces`（前の札）で伝え、
    engine はそのハンドを `needs_review` にする。
  - `release_sec=None` / `commit_sec=0` は従来の解釈（最初に見えた瞬間に確定・差し替えは明示の
    訂正だけ）。`tools/probe_pcsc.py` の検査はこちら。
- **位置割り当ての全解放は「新ハンド」、1 位置の解放は「明示のミスディール訂正」** **MUST**
  ...
  - **ミスディール訂正**（1 位置だけ）: `RFIDThread.forget_board_position(index)` /
    ...
    `correct_board`/`correct_seat`）で解放する。v1.5 の自動の差し替えで直らない形（1 枚だけの
    差し替えを急ぐ等）の手段として残す。解放後に載っているカードは再発火し、位置を持つ札は同じ位置に
