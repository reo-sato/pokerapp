...
- **ハンド内の位置は append-only** **MUST**（ISSUE-0026）。**一度与えた位置は、そのカードが
  盤上から消えても解放しない。** ポーカーではハンド中にボードのカードが減ることはなく、host 側の
  board（`integration/engine.py` の `_board_positions`）も **カードが外れても縮まない**
  （ハンド確定までボードを保持する設計）ので、両者を同じ規則にして構造的にずれないようにする。
