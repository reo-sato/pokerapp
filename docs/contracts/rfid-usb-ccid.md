...
- **位置の解放は「全 board reader の UID 和集合」で判定** **MUST**（ISSUE-0025）。
  reader 単位で解放すると、重なり配置で片方の台から消えただけで位置を失う。
  席 reader の抜き差しは board に影響しない。
- **位置割り当てのリセットは「新ハンド」だけを同期点にする** **MUST**（ISSUE-0026）。
  ボードが空になっても番号を振り直してはいけない。host 側の board（`integration/engine.py` の
  `_board_positions`）は **カードが外れても縮まない**（ハンド確定までボードを保持する設計）ので、
  RFID 側だけがボードが空になった時点で 1 番から振り直すと、置き直したときに engine の古い札と
  混ざって **同じ札が 2 か所に並び、枚数が水増しされてストリートが誤って進む**。
  新ハンドでは位置・記憶・board reader のデバウンス状態をまとめて落とす（盤上に残っている
  カードは改めて 1 番から検出し直す）。運用手順は「ハンドが終わったらボードを下げて新ハンド」。
