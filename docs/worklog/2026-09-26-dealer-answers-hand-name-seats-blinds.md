...
- 新規 67 件（役名 29 / 席・ブラインド・ボタン 21 / 確認型の発話 17）+ 更新 4 件（control queue の set_blinds /
  sit_out / sit_in / set_button、staff API、CLI の空席、to/by）。全体 1614 passed / 5 skipped、ruff clean。

## 仕様書（sprc_v4.docx）との突き合わせ（オーナーの指摘: 設計書・CLAUDE.md も参照すること）

- **FR-05b**「フォールドやバストアウトで欠番が生じた場合、アクティブ席の中で次の席へスキップする」— 席の参加・休みと
  スタック 0 の扱いは仕様どおり（質問 5 は仕様に答えがあった）。CLAUDE.md のボタン節の「Out of scope（着席/離席に
  伴うボタンの飛び越し）」を実装済みに改めた。
- **FR-05g / §9**「ディーラーボタン手動移動」+「直前ハンドとボタン位置が 2 席以上ずれていれば通知」— `button <席>`
  （CLI）/ control `set_button` を追加。通常の進み方と違えば知らせる（GUI のボタンは未実装のまま）。
- **FR-16/17 / §7 確認発話**「〜ですか？」は PENDING で、確定発話が無ければ発火しない — `is_question` で疑問形を
  アクションにしない（ADR-0056 が「未実装」と記録していた項目）。PENDING→確定の 2 段は、確定の発話だけを読めば
  観測上同じになるので持たない。
- **FR-33** ブラインドはセッション設定 — 今回の `blinds` はその additive な拡張（レベル上昇はオーナーの計画）。
- **§7** の推奨発話（「シート N ウィナー」「オールイン〔金額〕」「ポット〔金額〕」）は店舗の実際の発声（役名を言う・
  額やポットは言わない）と違う。仕様は推奨、実装は店舗の発声に合わせる（usage.md の「レイズの金額は最終的な
  「〜まで」の額」は質問 7 の答えでもあった）。
- **§8.3 / §10** フォールドはリーダーからの消失（`fold_absent_threshold`）、ショーダウン中の消失はフォールドにしない、
  距離・角度による誤消失はしきい値で調整 — 3 秒の不在判定 + ベッティング終了後は離脱をマックにしない、と同じ。

## 無言の連続（オーナーの追加情報: 同じアクションの 2 回目以降は言わない）

3 人の卓で「チェック、ベット 600」+ 札の離脱 2 つが、A: 席4 x / 席5 x（無言）/ 席6 b / 席4 f / 席5 f と
B: 席4 x / 席5 b / 席6 f / 席4 f のどちらか — 音声だけでは区別できない。

- 修正前: B は正しく記録、A は席5 のベット（誤）のあと席4・席5 の離脱が手番でないため止まり、次の配布まで
  確定しなかった（仮の勝者）。
- 修正後: A は席5 の離脱（ベットした人に手番は無い = 矛盾）で組み直し、席5 の無言チェック・席6 のベット・
  席4 と席5 のフォールド、席6 の勝ち（ポット 1200、要確認なし）。B は変わらず。全員がコールしてラウンドが
  終わる C（席5 と席6 のどちらがベットしたか）は決められず、ベットを `silent_run_ambiguous(席5・席6)` で要確認。
- 仕組み: `_run_hyps`（仮説: 仮の席・候補・チェックポイント）/ `_prune_run_candidates`（フォールドした席を外す）/
  `_contradicted_run` + `_reinterpret_run`（`_run_override[index] = k` を置いてその入力から流し直す。
  `_retract_departure` は根拠の席の札が戻ったら解釈も捨てる）/ `_settle_run_hyps`（ラウンドの終わりで候補が
  2 つ以上なら要確認）/ `_imply_action` の `silent_repeat`（直前に言われたのと同じ = 要確認にしない）/
  `_walk_to_sensed`（席・ポジションを言ったベットの間の人は無言の連続かフォールド）。
- 記録した leave / street の入力で replay も同じ結果になる（`test_silent_runs.py`）。
- 残る限界: 全員がコールしてラウンドが終わった場合の「誰がベットしたか」は音声と札では決められない
  （ベット / レイズだけポジションを付けて言えば決まる = 仕様 §7 の推奨発話）。
- オーナーの指摘（ベットに全員が降りると勝った人は素早く札を投げる）: 仮のベットの席の離脱を矛盾とみるのは、
  そのときほかの候補の席にまだ札があった場合だけ（`_had_cards_at`。候補の中で最後に離れた人がベットした人）。
  勝った人の離脱は「勝ってポットを取った」として要確認にしない（リバーは役名・「ショーダウン」を待ってから
  確定 = 「コール」の聞き落としの可能性）。「フォールド」と聞こえて札が残っている席は、札が離れたらその発話の
  時刻のフォールド（`_spoken_folds`, 15 秒以内）。`test_silent_runs.py::TestWinnerToss`。
