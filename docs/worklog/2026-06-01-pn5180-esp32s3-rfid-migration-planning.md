...
- 方針 ADR（ADR-0007）を Accepted で起こす。
- firmware ↔ Python の API 契約固定を ISSUE-0006 で追跡開始する。
- CLAUDE.md（プロジェクト概要 / 技術スタック / 実装状況 / ディレクトリ docstring）を新ハードに追従。
- 既存コードは触らない（PCSC 経路含め、判断は ADR にのみ記録）。

## Changed Files

- `docs/adr/0007-migrate-rfid-to-pn5180-esp32s3-and-canonical-http-transport.md` — 新規。
  HTTP transport を canonical、PCSC を legacy 降格、JSON 契約は不変。
- `docs/issues/0006-pn5180-firmware-http-contract.md` — 新規。firmware ↔ Python API 契約の
  open question を登録。
...
- `docs/decision-log.md` — ADR-0007 / ISSUE-0006 を Index に追加。

## Expected Behavior

- 既存コード・config・テストは **一切変更しない**（docs-only コミット）。
- 既存テストはすべて従来通り通る。
- ADR-0007 が「HTTP canonical / PCSC legacy / JSON 契約不変 / UID 4-7-8B 許容」を明文化。
- CLAUDE.md 上で PN5180 + ESP32-S3 が「現時点で採用予定の RFID 構成」として読める。
- ISSUE-0006 が `tag_id` 書式・8B UID 検証・error/heartbeat 等の未確定事項を register する。

## Implemented Behavior

期待どおり実装:

- ADR-0007 Accepted（Alternatives A/B/C で両系統維持・JSON 即時変更・PCSC 即時削除を却下、
  HTTP canonical + PCSC legacy 降格を採用）。
- ISSUE-0006 Open（firmware 仕様未確定 5 項目をリスト）。
...
- [ ] `card_master.normalize_tag_id` の 8B UID 許容確認 + 単体テスト追加（**ADR-0007 follow-up**）。
...
- [ ] PN5180 firmware（別 repo）の API 契約サンプル payload を ISSUE-0006 に貼って固定。
- [ ] PCSC 経路の完全削除可否判断（別 ADR）。
- [ ] 将来: `docs/contracts/schemas/rfid_event.schema.json`（POST /rfid body の contract 化）。

## Related ADRs

- `docs/adr/0007-migrate-rfid-to-pn5180-esp32s3-and-canonical-http-transport.md` — 本タスクの中心判断。

## Related Issues

- `docs/issues/0006-pn5180-firmware-http-contract.md` — firmware ↔ Python 契約の未確定事項。
