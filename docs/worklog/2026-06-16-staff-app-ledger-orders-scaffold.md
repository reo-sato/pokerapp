# Worklog: staff iPad アプリ — 会計/注文タブの scaffold（WS4, ADR-0035）
...
ADR-0035 の最優先 2 機能（会計 / 注文）について、新規 `staff/`（Expo/RN）アプリを mock 先行・
...
- `staff/src/api/httpRepository.ts` — `HttpStaffRepository`（staff API を fetch、Bearer token。
  `listSessions` は ADR-0036 §B 未実装で `not_implemented`）。
...
- `HttpStaffRepository` は会計/注文の staff endpoint に 1:1 対応。`listSessions` のみ未実装
  （`GET /api/staff/sessions` = ADR-0036 §B）で `not_implemented`、`listPlayers` は当面 `GET /api/players`
  を流用。
...
- [ ] 座席タブ / ハンドタブ（不足 staff API = ADR-0036 §B/§C 実装後）。
- [ ] HTTP `listSessions`（`GET /api/staff/sessions`, ADR-0036 §B）。現状 mock のみ。
- [ ] 確定済 settlement の再取得 API が無く、commit 後の支払状態は画面 local state 保持
      （画面離脱で消える）。GET committed settlements は ADR-0036 で検討。
...
- `docs/adr/0035-staff-ipad-app-touch-frontend.md` — staff iPad アプリ（本 scaffold の設計）
- `docs/adr/0036-staff-api-session-seat-handlogger-expansion.md` — 不足 staff API（後続）
