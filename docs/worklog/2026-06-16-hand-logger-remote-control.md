# Worklog: hand logger 遠隔制御 + staff アプリ ハンドタブ（WS4 §C, ADR-0037）
...
ADR-0036 §C（hand logger 遠隔制御）を実装する。プロセス境界（録音 = 別プロセス常駐）を越えるため、
ISSUE-0020 Q1 の第一候補 **append-only control-command queue** を採用（ADR-0037）。staff アプリに
...
- `docs/adr/0037-hand-logger-remote-control-command-queue.md`（本実装）
- `docs/adr/0036-staff-api-session-seat-handlogger-expansion.md`（§C を ADR-0037 へ）
