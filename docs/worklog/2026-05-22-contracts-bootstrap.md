...
- 並行開発の主リスクは **contract drift**（ISSUE-0003）。これを test で検知するには、schema と
...
- `docs/issues/0003-parallel-dev-contract-drift.md` — Phase 0a の部分緩和を追記、Status 更新。
- `docs/issues/0004-hand-id-int-vs-cross-app-string.md` — 新規 issue (Open)。
- `docs/decision-log.md` — ADR-0005 / ISSUE-0004 を index に追加。
...
- **hand_id の int vs cross-app 文字列契約の不整合** を発見。共有 ID 原則は「cross-app では
  ...
  単体運用前提の設計差。shared-ids.md に既知不整合として記載し、**ISSUE-0004** に分離して
...
- [ ] `session_id` / `hand_id` の cross-app 形確定（ISSUE-0004, S2）。
...
- `docs/issues/0003-parallel-dev-contract-drift.md`（部分緩和）
- `docs/issues/0004-hand-id-int-vs-cross-app-string.md`（新規）
- `docs/issues/0001-point-balance-source-of-truth.md`（S3 の gate）
