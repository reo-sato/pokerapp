# Worklog: staff アプリ ledger reversal UI + entry 一覧 read API（WS4, ADR-0038 §A）
...
ADR-0038 §A の残項目 = staff アプリの **ledger reversal UI** を実装する。reversal は対象 entry を
...
- [ ] ハンドタブ（hand logger 遠隔制御 = ADR-0038 §C / ISSUE-0020）。
- [ ] seat batch の idempotent-replace（core の hand seating クリア method 後）。
- [ ] 確定済 settlement の GET（commit 後の支払状態の再取得）。

## Related ADRs

- `docs/adr/0038-staff-api-session-seat-handlogger-expansion.md`（§A）
