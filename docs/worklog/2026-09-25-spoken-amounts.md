...
## 追記: 2 ハンド目（真のアクションはオーナー提供）

真: PREFLOP raise 600(6) / fold(4) / raise 2500(5) / call(6)、FLOP bet 1500(5) / call(6)、TURN check / check、
RIVER bet 1000(5) / raise all-in(6) / fold(5)。

- 「ロープヘッグ」（おそらく「ロッピャク」= BTN のレイズ 600）が読めず、以降の手番がすべてずれた
  （フォールドが席6、2500 が席4 に付いた）。フロップ・リバーのアナウンスは 1 つもアクションにならなかった
  （「ご視聴ありがとうございました」等の幻聴・「ポループ」「オーリー」）。短い単語の書き起こしが根本の問題。
- 直したもの: 額を BB の倍数に丸めていた（2500 → 2400）→ チップの最小単位（gcd(SB, BB)）に。`LegalContext.chip`
  を追加。「チェック、アランド」のチェックアラウンド。仮名だけの額（「ロッピャク」）。「お楽しみに」の幻聴。
- 設計の見直し（一般的なディーラーの発声のままで成立させられるか）は別途監査を受けて提案する。
