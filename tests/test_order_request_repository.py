...
Phase M5 (ADR-0015) — 注文リクエストの core テスト。

検査対象（docs/contracts/ledger.md § 注文リクエスト / error-shapes.md）:
...
class TestCreateRequest:
    def test_create_pending(self, env):
        ...
        # ledger には何も書かれない（staff-in-the-loop, ADR-0015 §2）
...
class TestConfirmReject:
    ...
    def test_confirm_failure_keeps_pending(self, env):
        """ledger 側 validation 失敗（closed session）では pending のまま。"""
        sid, pid = env["session"].session_id, env["alice"].player_id
        r = env["orders"].create_request(sid, pid, "ビール", 1)
        env["sessions"].close_session(sid)
        from core.ledger_repository import LedgerSessionClosedError
