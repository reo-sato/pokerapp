...
def test_record_appends_jsonl(tmp_path: Path):
    ...
    rec.record(RFIDEvent(
        tag_id="04:CC", card="Qs", reader_id="seat_3", role="seat",
        seat=3, timestamp=2.0, raw_tag_id="04CC", board_index=None,
    ))
    lines = path.read_text(encoding="utf-8").strip().split("\n")
    assert len(lines) == 2
    assert json.loads(lines[0])["type"] == "audio"
    assert json.loads(lines[1])["type"] == "rfid"
