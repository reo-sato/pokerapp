...
viewer API の HTTP テスト (ADR-0017, docs/contracts/viewer-api.md)。
...
def test_player_ledger_entries_and_summary(env: dict):
    """会計参照 endpoint（ADR-0017）: entries は schema 適合、summary は settlement 由来の
    中間集計（verify-v1 ledger, ADR-0016）。"""
    sid = env["session"].session_id
...
class TestOrderRequests:
    """M5 (ADR-0018): 注文リクエスト write path。"""
    ...
    def test_post_creates_pending_without_ledger_write(self, env: dict):
        ...
        # ledger には書かれない（staff-in-the-loop, ISSUE-0019 の回帰固定）
    ...
    def test_post_readonly_mode_503(self, env: dict):
        """単独 --viewer-api（orders_writable=False）では注文 write を受けない（ADR-0018 §3）。"""
        sid, pid = env["session"].session_id, env["alice"].player_id
