...
// 遅れるのは「既に札がある reader に増えた札」＝ 席の 2 枚目 / flop の 2・3 枚目だけで、
// ディーラーが配る間隔より十分速い。
//
// **位相ずらしを入れた後の値（2026-09-11, ISSUE-0021 実機フィードバック 8）= 6**。
// 完全確認を行うのは `(poll 周回 + reader index) % N == 0` の reader だけなので、1 周に
// ceil(11/N) 台しか完全確認しない。実機 11 台満載の実測（簡略 21.6 ms / 完全 45.0 ms / 台）から:
//   N=3 → 1 周 308〜331 ms（avg 323）   N=4 → 284〜308 ms（avg 302）
//   N=5 → 284〜308 ms（avg 289）        **N=6 → 261〜284 ms（avg 281）← max が初めて 300 ms 未満**
// N を 7 以上にしても ceil(11/6)=2 から減らないので max は縮まない（avg だけ僅かに下がる）。
// 遅れの上限は N 周 ≈ 6 × 0.28 s ≈ 1.7 s。速い検出を優先するなら 3 に戻す（max 331 ms）。
#define PN5180_FAST_CONFIRM_EVERY 6
