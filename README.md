...
> ⚠️ 本アプリは現在 **v1 開発中**です。店舗の Windows PC には **`install.cmd` をダブルクリック**
> （または PowerShell に 1 行）で入ります — Python が無くても可（ADR-0057,
> [インストール手順 §0](docs/installation.md#0-windows-ワンステップ店舗-pc)）。開発者は「ソースから実行」も可。
